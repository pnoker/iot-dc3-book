...
6. 严格遵守格式规范和术语规范
7. 知识库不是唯一来源，也不是内容边界；它只用于校准术语、事实、标准和技术边界，不能决定本书结构、观点和方法论
8. 证据优先：统计数字、年份分界、版本号、标准状态、成本、性能、市场规模、项目效果、企业案例等硬事实，必须能在参考资料或研究资料包中找到明确依据
9. 没有证据的硬事实不要写；需要讲解时改为定性分析、作者归纳、工程判断、假设场景或方法论步骤，并明确标注“假设场景/示意”
10. 扩充篇幅时优先增加原理解释、工程权衡、检查清单、流程步骤、风险分析、对比表和实践清单，不得用虚构数字、虚构真实项目或伪来源凑字数
11. 图表只在架构、流程、演进路径、关键对比确有必要时使用；不要为了满足形式在每个小节硬塞图
12. 不模仿旧教材的段落组织、定义口吻、讲课式语气、例题风格或原文表达
13. 去除 AI 腔，写得像人写的：不用"在当今…时代""随着…的发展""综上所述""值得注意的是"这类套话开场和过渡；不把内容硬凑成"不仅…而且…更…"的排比三连；不在每段结尾加"这为…奠定了基础""具有重要意义"这类空心总结句；敢下明确判断和取舍，而不是一味"既要…也要"的中立平滑；长短句交错、段落有疏密，不要句句等长节奏均匀；克制加粗和冒号列表，用论述代替罗列
...
- 知识库不是唯一来源，不能把知识库当作全书内容上限；它只是事实校准和引用核查的参考层。
...
- 如果资料中没有明确依据：删除具体数字，改为定性表述、工程判断、作者归纳或方法论步骤；或明确写成“假设场景/示意案例”，且不要给出会被误认为真实项目数据的精确数值。
- 不要编造 Gartner、IDC、IoT Analytics、Cisco、3GPP、企业项目、城市项目等来源；资料包没有的来源不得写入正文。
- 可以用行业常识、工程经验、架构推理、表格、流程、检查清单、风险矩阵、伪代码和实践清单扩充内容；这些内容不要求逐句来自知识库，但不能伪装成有来源的硬事实。
- 章节宁可少一些硬数字，也不能出现无法核验的硬数字。"""

_REFERENCE_ROLE = """## 参考资料角色（必须遵守）
- 本地知识库多为较早期教材和学术资料，只用于校准术语、事实、标准和技术边界。
- 知识库不是唯一来源，不得把知识库当成写作素材库、章节结构来源或观点来源。
...
请输出该小节的 Markdown。必须使用合适的 ## 或 ### 标题，不要输出整章标题。"""
    ...
    def _figure_contract_prompt(self, state: BookState) -> str:
        ...
        professional_fields = illustrations.get("professional_fields")
        if not isinstance(professional_fields, list):
            professional_fields = [
                "audience_takeaway",
                "visual_focus",
                "design_level",
        ...
        professional = "、".join(str(item) for item in professional_fields)
        example_block = f"\n\n字段格式参考：\n{example}" if example else ""
        return f"""
- 如果本小节输出 `{marker}` 代码块，必须逐项包含这些字段，字段名不能省略或翻译：{required}。
- 同时必须按出版级图表 brief 补齐这些专业字段：{professional}。
...
- `components` 必须是结构化 YAML 对象列表，每项包含 id、label、type、subtitle、group、priority、shape；label 必须短，不超过 14 个汉字。
- `components` 是图中真实的实体节点（如“感知层”“平台服务”“设备网关”）；绝不能把“数据流/控制流/内部流”这类**关系或流向**当成节点写进 components——它们只属于 `connections` 或 `relationships`。
- `connections` 必须引用 components.id，每项包含 from、to、label、style、direction；禁止把多条关系写进一句话。
- `regions` 只表达职责边界、系统边界或治理边界；`visual_focus` 必须说明主链路、关键风险或高亮路径。
- `audience_takeaway` 写这张图具体让读者得到的一个结论，必须贴合本图内容，禁止套用“主链路、责任边界和工程取舍”这类与图无关的通用话术。
- 禁止使用“节点1/节点2/最右侧/container/service/user”这类占位标签；解释性文字放入 `callouts` 或正文，不塞进节点。
{example_block}"""
