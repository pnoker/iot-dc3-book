...
- [ ] 积压告警阈值：RabbitMQ 队列深度超过 10,000 且持续 60 秒时告警。
- [ ] 数据库写入慢查询：监控 `data_point_value` 表的 `track_io_timing`，设置 PostgreSQL `log_min_duration_statement = 200ms`。
...
audience_takeaway: 读者应理解AGI时代物联网架构演进中的主链路、责任边界和工程取舍。
