...
Combined, these two mechanisms can push typical standby current down to extremely low levels. One example: a smart water meter powered by two AA alkaline batteries, reporting once a day at a medium coverage level — from a circuit-board design perspective, the battery life can support several years. Real-world battery life, however, is affected by many factors — reporting frequency, battery capacity, ambient temperature, silicon process, and the power-saving parameters the module vendor supplies (such as eDRX cycle settings) — and figures differ noticeably across datasheets, so no precise numbers are given here.

| Parameter | Typical range | Notes |
|---|---|---|
| Channel bandwidth | Extremely narrow, single-frequency | Not dynamically allocated; consistent with the narrowband design, uses only one LTE resource block |
| Uplink peak rate (early releases) | Low, basic messaging traffic only | Defined in early releases; later releases can raise the rate |
| Downlink peak rate | Same order of magnitude | Symmetric architecture, but the downlink link budget is more generous |
| Coverage levels | Multiple | Higher levels mean more repetitions — deeper coverage, but greater latency and power consumption |
| Maximum coupling loss improvement | Significant gain over baseline LTE | A qualitative description of penetration; the actual value depends on band, transmit power, and receiver sensitivity |
| Standby current (PSM/eDRX enabled) | Extremely low | Depends on chip implementation, system clock design, and whether an RTC is retained |
| Operating bands | Multiple LTE bands | Operators can choose low and mid bands for deployment |
...
### 4.1.2 LoRa and LoRaWAN: Long-Range, Low-Power Private Networks
...
In recent years the WiFi Alliance has released a version operating in the sub-1 GHz band, trading peak rate for longer coverage and lower power — though its terminal ecosystem and chip supply are not yet as mature as products for the main bands. In another direction, newer versions of the standard introduced Orthogonal Frequency Division Multiple Access (OFDMA) and Target Wake Time (TWT); the latter lets devices schedule sleep windows, reducing light-sleep power while remaining standards-compatible — genuinely valuable for battery-powered cameras and door locks, though still a wide gulf from BLE-class ultra-low power.
...
Compared with BLE, Zigbee offers greater node capacity in industrial-grade deployments (hundreds to a thousand nodes in mesh mode), and its ZCL definitions are more detailed. The bottleneck is that almost every Zigbee device must reach the internet through a coordinator — the gateway is not optional; it is an intrinsic feature of the architecture.
...
| Range (indoor) | Tens of meters | About ten meters | Ten to a hundred meters |
...
**Class 3: high mobility, latency-sensitive.** AGV scheduling, remote control, industrial robot coordination. The 5G uRLLC slice is designed for millisecond-level deterministic control, but its engineering cost is still a sledgehammer-to-crack-a-nut proposition — if the device's motion path is fixed and latency requirements are lenient, WiFi or even wired Ethernet is more economical. 5G mMTC overlaps functionally with NB-IoT, but NB-IoT's module cost and network coverage maturity currently far outclass 5G mMTC.
...
#### The 2026 View of Connectivity Evolution: RedCap, NTN, Wi-Fi 7, Matter/Thread, and TSN
...
- **Matter and Thread**: Matter defines the application-layer device model and commissioning flow; Thread is merely one bearer. If the goal is interoperability with consumer ecosystems, Matter is a workable entry point; industrial protocol interoperability still rests mainly on OPC UA and Modbus.
