...
#### The Meaning and Coverage of the 28 Protocol Drivers

What IoT DC3 calls "28 built-in protocol drivers" is not a cap on the count — it is the number of protocol adapters already packaged in the current official release. The driver framework is fully open: developers can build arbitrary custom protocol drivers on top of the Driver SDK, and once registered, the platform recognizes them automatically. These 28 drivers cover common fieldbuses (Modbus RTU/TCP), industrial wireless (LoRaWAN, NB-IoT, BLE), PLC protocols (Siemens S7, the Mitsubishi MC protocol), SCADA data formats (OPC UA), and even database drivers (querying data directly out of a database as device input) and virtual test drivers (simulated devices for development and testing). Set against the protocol fragmentation discussed in Section 4.2, IoT DC3's strategy is not to "invent a new standard that wipes out fragmentation" but to "digest fragmentation with a unified driver framework." One driver per protocol encapsulates its details and exposes a consistent read/write interface to the outside.
