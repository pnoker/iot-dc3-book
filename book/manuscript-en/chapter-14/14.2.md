...
Table 14-1 shows the responsibilities and typical risks of every component along the complete path that an uplink point value travels from device to database.
...
| Message delivery | `DriverSenderService` → RabbitMQ | Publish `PointValue` to exchanges such as `topic_point_value` | Batched publishes to raise throughput; persistent messages (delivery mode = 2) | Queue backlog when the production rate exceeds the consumption rate; RabbitMQ disk filling up |
| Async consumption | Data's `PointValueReceiver` | Pull messages from RabbitMQ and, by threshold, either save directly or buffer into `PointValueJob` | Multi-threaded consumption with per-thread acks; configurable buffer threshold (for example, 100 messages or 1 second) | Slow consumption causing backlog; a single failed record rolling back a whole batch |
...
| Persistence | Data → PostgreSQL repository | Write history records into the `data_point_value` table | Batch inserts with PreparedStatement; time-partitioned tables | Write throughput bounded by PostgreSQL disk IO; index bloat |
