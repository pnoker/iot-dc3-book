...
Fitting a trained neural-network model onto an MCU with only tens of KB of memory is not a simple copy-and-paste job. Models exported by mainstream deep-learning frameworks (TensorFlow, PyTorch) typically use 32-bit floating-point (float32) weights and activations. A 100 KB float32 model can consume close to 400 KB of Flash on an MCU. Two things are needed: quantization and pruning.
