...
| Soil moisture/temperature | Decagon EC-5 | VWC/temperature | 0–100% VWC / -40–60 °C | ±2% VWC in mineral soil | Analog/digital | ~15 mA while measuring | Medium |
...
A typical standalone power module contains three parts: a **solar panel** (polycrystalline or monocrystalline silicon), a **charge-management circuit** (with maximum-power-point tracking), and a **rechargeable battery** (lithium-ion or lithium iron phosphate). The sizing calculation is not complicated: first compute average daily consumption (mA·h), then multiply by the local "consecutive overcast-day tolerance" (usually set at 3–7 days) to obtain the required battery capacity; then compute the compensating panel power from average sunshine duration (for example, about 4–5 peak sun hours in the North China region). In winter, when light is insufficient and low temperature reduces battery discharge efficiency, simply raising the panel power and battery capacity provides the backstop.
...
To make the deployment logic above easier to grasp visually, Figure 12-1 shows the topology of a typical environmental sensor network.
