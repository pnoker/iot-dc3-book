...
**The approach of post-quantum cryptography.** The assumption that elliptic-curve digital signatures (ECDSA) — widely deployed on IoT devices — are vulnerable to quantum computing has been repeatedly validated by the academic community. In 2024, NIST formally published the post-quantum cryptography (PQC) standards FIPS 203 (ML-KEM), FIPS 204 (ML-DSA), and FIPS 205 (SLH-DSA), with the lattice-based ML-KEM/ML-DSA now official standards. For microcontroller-class IoT nodes, these algorithms' key sizes and signature-computation overhead are hard constraints, and the industry is exploring integration paths for lightweight post-quantum signature schemes (such as hash-based XMSS) inside TEEs or secure enclaves. Engineering judgment: for the foreseeable period, blockchain-IoT projects should provision PQC-upgradeable key-storage modules at the hardware level, avoiding a migration dead-end after large-scale deployment.

**Native integration of 6G networks and blockchain.** Early ITU-T discussions on IMT-2030 already include proposals to embed distributed trust mechanisms into the network protocol stack. 6G is designed to support machine-to-machine collaboration at extremely low latency, which requires trust to be a native network capability rather than an overlay layered on top. Blockchain (or its DAG variants) may exist as a "network-native trust layer" — dedicated consensus-node resources allocated through network slicing, or device locations anchored to on-chain identities through integrated sensing and communication. 6G standardization has entered substantive progress (3GPP Release 21 started in 2025, the ITU-R IMT-2030 framework is established, and the specifications are expected to freeze around 2029), but network-native trust remains an open research topic — long-term architecture planning should reserve lightweight cross-domain identity interfaces.
...
- **Standards**: W3C DID Core 1.0; the IOTA Tangle white paper; the official Hyperledger Fabric documentation.
- **Sample papers**: A. Dorri et al., "Blockchain for IoT: A Survey", IEEE COMST, 2019 (a survey reference); J. D. Choi et al., "Blockchain for AI: A Survey", 2020.
