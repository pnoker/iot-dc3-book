...
Table 10-3-1: Feature comparison of mainstream industrial time-series databases
...
The power of a time-series database depends not only on the storage engine but even more on a sensibly designed data model. In IoT DC3 practice, one time-series record is modeled as a **PointValue** — each value carries four fixed attributes:
