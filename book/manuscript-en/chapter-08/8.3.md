...
> **Reading note**: The TLS/DTLS protocol specifications are covered in full in Chapter 9, and transport-encryption practice for MQTT/CoAP scenarios was already developed in the device-access discussion of Chapter 4. This section takes the security perspective — it discusses transport-encryption engineering from the angles of threat model, certificate management, and key rotation, rather than the protocols themselves. If you need the complete sequence of the TLS handshake, see Figure 9-4 in Chapter 9; broker configuration examples for MQTT over TLS are in Chapter 4.
...
*Table 8-3 Suitability comparison of typical cryptographic algorithms*

In engineering practice, the recommended suites are `TLS_ECDHE_ECDSA_WITH_AES_128_CCM` (RFC 6655) or `TLS_ECDHE_ECDSA_WITH_CHACHA20_POLY1305` (RFC 7905). Keep the certificate chain no deeper than two levels — every extra certificate the device transmits during the handshake adds several hundred bytes of traffic, which on extremely low-bandwidth links such as LoRaWAN or NB-IoT can blow past the MTU or visibly extend the time to complete the handshake.

#### An Nginx Configuration Example

Below is a typical Nginx reverse-proxy configuration providing TLS termination for an MQTT broker. The paths and cipher suites in the example must be adjusted to the actual security baseline.
...
In practice, the three schemes are often mixed. The MQTT 5.0 specification carries a "session expiry" in the CONNECT message which, combined with the maximum processed sequence number maintained at the broker, is one such hybrid of timestamp and sequence number. For CoAP, the IETF standardized OSCORE (Object Security for Constrained RESTful Environments), which encrypts and wraps the CoAP message body at the application layer, protecting the timestamp and the sequence number together inside the encrypted payload — a dual guarantee of "integrity plus replay protection."
