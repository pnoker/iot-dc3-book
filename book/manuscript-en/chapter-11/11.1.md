...
The four-layer architecture is the vertical "skeleton," but intelligent transportation also needs horizontal coordination — the "vehicle-road-cloud" closed loop. Vehicles upload real-time position and motion state through the connected On-Board Unit (OBU), Road Side Units (RSUs) synchronize signal, speed-limit, and incident information, and the cloud platform performs global scheduling and prediction, sending results back down to vehicles through the roadside network. The intelligent connected vehicle plays a dual role here: it is both a data source and an actuator. Understanding the layered architecture and "vehicle-road-cloud" collaboration lays the groundwork for the discussions that follow — V2X communication technology selection and roadside device deployment.
...
DSRC is a "closed but mature" technology stack. The United States, Europe, and Japan ran extensive test validation from the 1990s onward, but its standardization is frozen, the physical layer cannot break through, and it cannot exploit the scale advantages of cellular network infrastructure. For newly built urban districts where every critical intersection can be equipped with an RSU, DSRC is a low-barrier, mature choice; but once RSU density cannot cover every scenario, DSRC's V2V communication in RSU-free areas turns fragile, because its distributed contention mechanism suffers channel congestion at high vehicle density.
...
The later 5G NR releases further reduced latency on the PC5 interface, support higher throughput, and add more flexible scheduling. The PC5 interfaces of LTE-V2X and NR-V2X are compatible on the same band, enabling smooth evolution.
...
| Evolution | Frozen | Smooth evolution from LTE to 5G |
...
- **V2I (vehicle-to-infrastructure)**: vehicles receive signal states, speed-limit advisories, and variable message sign content. The RSU periodically broadcasts signal phase and timing (SPAT) messages; vehicles decode them and estimate the remaining green time.
...
- **Spectrum availability**: the 5.9 GHz band is allocated differently across countries. If the band has already been assigned to C-V2X locally, deploying DSRC runs into interference or compliance obstacles. The reference material stresses that "supporting both legacy and future systems is the real challenge" — band compatibility is the first-order problem of the transition period.
...
Data exchange between the RSU and the intersection signal controller is the most fundamental and most valuable scenario class in intelligent transportation. The interworking workflow divides roughly into five steps:
