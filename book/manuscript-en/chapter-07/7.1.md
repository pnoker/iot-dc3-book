...
**The second: the leap in model capability.** In recent years, large language models have leapt from "text conversation" to "tool calling." Traditional IoT intelligence relies on rules and classification/regression models, whereas today's LLMs can take a natural-language instruction such as "set the feed valve on Line 2 to a lower opening" and reason out which API to call, what parameters to pass, and even perform boundary checks. This capability is a natural match for the IoT's command-intensive character. IoT DC3's approach is pragmatic: through Spring AI, tool calling becomes an ordinary Java method call, letting the model's comprehension plug straight into the platform's existing business logic. The model never needs to perceive underlying protocol differences (Modbus, OPC UA, MQTT), because those differences are already shielded by the platform's device abstraction layer.

These two driving forces point to one conclusion: AIoT has moved from concept to engineering practice. The following subsections take this apart one by one — the concrete role of large models in the IoT (Section 7.1.2), how agents achieve autonomous decision-making (Section 7.1.3), and the key technologies that let models reach out and touch the physical world — RAG, Tool-Calling, and MCP (Sections 7.1.4 and 7.1.5).

### 7.1.2 The Role of Large Models in the IoT: From Perception to Cognition

The rule engine has run for years as the core analytical instrument of the traditional IoT: if temperature crosses a threshold, raise an alarm; if a device goes offline, send a notification. The mechanism's boundary is clear — it handles only explicitly defined discrete rules. When an operator faces a compound judgment such as "the temperature in pump house 2 is 5 degrees higher than the same time yesterday, yet the load has fallen," the rule engine can output nothing more than "temperature over limit." It cannot understand the semantics of "the same time yesterday," nor reason about the causal link of "temperature rising abnormally while load falls."

Once large models enter, this ceiling breaks. The model does not merely recognize the number "temperature 86 °C"; it correlates context — season, load, historical trend, maintenance records — and then concludes "this is not normal fluctuation; it may be declining cooling-pump efficiency." The leap from **numerical judgment to semantic reasoning** is the core value large models bring to the IoT: machines evolve from knowing only how to "detect" to being able to "understand." This change does not wholesale replace the rule engine; it layers a cognitive capability on top of it — the rule engine keeps handling second-level responses and discrete events, while the large model handles the tasks that demand contextual understanding, fuzzy matching, and causal inference.
...
Input in industrial settings is not limited to text and numbers. A camera catches an abnormal indicator light blinking on an equipment panel; an operator photographs it, posts it to a group chat, and asks "what does this mean?" — a traditional platform cannot process such input. Multimodal large models (mainstream examples include OpenAI's GPT-5 and Anthropic's Claude 4.5) accept image and text input at the same time: the blinking-light pattern in the photo, the gauge needle's position, the scorched color of a wire — all of it can be brought into the reasoning scope.
...
A rule engine that detects an alarm can only say "temperature exceeded 85 °C." A large model can do more: pull the point's trend over the past 7 days, compare it against the same period, consult the maintenance logs, and output a logically grounded diagnosis — "this temperature rise is twice the usual rate; combined with the last two shutdown records, it may be bearing wear reducing cooling-pump efficiency. Schedule a shutdown and inspection within 30 minutes, focusing on the cooling pump's current and outlet pressure."

This is the leap from **descriptive analytics** ("what is the temperature now") to **diagnostic analytics** ("why is the temperature high"), and on to **prescriptive analytics** ("what should be done next"). The key foundation under this leap is tool calling — the large model itself holds no permission to read live data; it must obtain devices, drivers, thing models, points, and values through the 8 Tool classes explicitly registered in the Agentic Center's current Provider, and only then synthesize a judgment and output a recommendation. The `CommandTool` and `EventTool` in the source code have not yet been added to the current Provider, so they do not count as default session capabilities.
...
| Alarm triggering | Fixed threshold checks, templated notifications after trigger | Root causes reasoned with context, generating diagnostic suggestions and handling steps |
...
The role of large models in the IoT can be summed up this way: they fill in a long-missing cognitive layer. Sensors acquire massive data and the rule engine renders fast verdicts, but "understanding context, generating suggestions, conversing with people" has always been missing. Large models fill exactly this gap, letting the IoT evolve from passive perception to active cognition — without replacing the existing real-time control logic. The next question goes one step further: how to host this cognitive capability in an Agent Runtime, and constrain probabilistic decisions into governable industrial execution.
...
- An **Agent** is the decision-making subject that judges the next action within a given context, skilled at understanding intent, marshaling evidence, and planning dynamically.
...
| **Agent** | The decision-making subject that dynamically selects the next action from the current Context | "What should be done right now?" | Should not bear persistence alone |
...
A Tool is a capability, not a complete task. "Query device status" and "write point value," for example, can be two Tools. MCP can expose them to external agents, but it will not automatically orchestrate them into a reliable maintenance procedure. Skill is this book's name for the unit of domain reuse: a "pump-house offline triage Skill," for example, can contain triage prompts, equipment-topology knowledge, three read-only Tools, and one human-confirmation Workflow. Frameworks do not yet agree on how Skills are named and packaged, so in engineering you must make explicit what a Skill contains — a label alone is not enough.

Workflow and Agent are not substitutes for each other either. Workflows suit processes with stable steps, clear responsibilities, and known failure compensation; agents suit tasks whose goal is clear but whose path must be chosen dynamically from on-site information. The combination industry commonly uses is: **the Agent chooses the path, the Workflow guards the critical steps, the Tool performs the atomic action, MCP connects external capabilities, and the Runtime manages the whole lifecycle.**
...
In this example, the model decides "what to investigate next," while the Runtime guarantees "under whose identity, up to where, whether execution is permitted, what happens on failure, and where the evidence is kept." Neither can be missing.
...
**Agent Runtime rollout checklist**
...
Once large models are plugged into IoT operations, they quickly run into two very real shortcomings. The first is the knowledge boundary: the moment a model finishes training, what it knows is already stale — the variable-frequency drive commissioned last night, the register mapping table updated just now, the standard operating procedure changed this quarter are all unknown to it. The second is the action boundary: however clever, the model can only output text and cannot put a command on the bus directly. Asked to "restart pump 3," it can only reply, "please log in to the platform, find pump 3 in the device-management screen, and click the restart button." Retrieval-augmented generation (RAG) and tool calling (Tool-Calling) each close exactly one of these gaps: the former lets the model answer questions with up-to-date material in hand, the latter lets the model actually operate equipment.
...
A typical dialogue for the combined workflow (an example):
...
3. **Step 2**: read with the SOP that `fault_code=0xE3` means "compressor over-current," and output a preliminary diagnosis
...
The combination of RAG and Tool-Calling means the large language model in IoT operations neither "answers out of thin air" nor "can only talk." The two mechanisms form the operational foundation of intelligent O&M, providing a double guarantee for later agent systems to achieve autonomous planning and execution. The view now rises from single tool calls to the system-integration level, to see how these capabilities are exposed to external AI agents through a standard protocol.
...
IoT DC3 currently exposes an MCP JSON-RPC entry at the Gateway's `POST /mcp`, declares only the **tools capability**, implements `initialize`, `ping`, `tools/list`, and `tools/call`, and accepts `notifications/initialized`. Resources and Prompts are optional capabilities in the MCP specification, but the current endpoint neither declares them nor implements the corresponding list/read/get methods.

Nor is the tool catalog a "Resource list" generated by scanning Spring AI `@Tool` methods. The `McpOpenApiAggregator` in Auth assembles Tool names, descriptions, and input schemas from static OpenAPI specifications; the Gateway's `tools/list` returns the catalog visible to the current caller based on the OAuth connection and the tool whitelist. Before every `tools/call`, the Gateway re-validates the bearer token, the connection context, and tool visibility, then forwards the call to the real REST backend.
...
**From MCP to A2A: interoperability among agents (outlook).** MCP answers "how does an agent call tools," while A2A (Agent-to-Agent, jointly proposed by several vendors in 2025) answers "how do agents discover one another, allocate tasks, and hand over execution" — the industry often compares the two as "MCP gives agents a pair of hands; A2A gives agents colleagues." A2A describes capabilities through Agent Cards and supports task delegation, and is expected to scale gradually from 2027 in cross-system, cross-organization multi-agent scenarios. The significance for the IoT is this: once a platform exposes device capabilities to external agents via MCP, agents of different kinds (equipment inspection, energy scheduling, production scheduling) still need A2A to coordinate the division of labor — precisely the embryo of an "Internet of Agents." A complete AI-native platform should plan both layers of interoperability, MCP (the tool layer) and A2A (the orchestration layer); IoT DC3 currently implements the MCP tool layer, and A2A orchestration belongs to the platform's evolution roadmap (see the outlook in Chapter 14).
...
- **Recall@k**: whether the top k results cover the evidence that should be hit;
- **MRR**: whether the first correct result appears high enough;
...
These metrics should be reported for the sparse-retrieval, vector-retrieval, hybrid-retrieval, and hybrid-plus-reranker baselines alike. Showing only the best scheme leaves the reader unable to judge whether the added complexity actually pays.
