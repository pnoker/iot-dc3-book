...
**Table 9-7 Comparison of semantic interoperability levels (a synthesis based on common engineering practice)**
...
**The current juncture: from platform semantics to device-interaction semantics.** MCP (see Section 9.5) is the newest extension of this evolution. It is not affiliated with any particular standards organization, but judging from its design, its device-capability description conceptually echoes the attribute expression of WoT Thing Description, its context management draws on oneM2M's subscription/notification pattern, and its transport layer reuses the existing security foundation. These convergent ideas have concrete practice in the IoT DC3 project, though this is not an official standardization statement. MCP's value lies in pushing semantic interoperability from the platform layer down to the granularity of "devices conversing directly with AI agents" — at registration, a device can carry a structured thing-model identifier and also attach a natural-language-readable capability description for AI agents to interpret dynamically. The following description is based on IoT DC3's design practice with MCP: MCP exposes device operating capabilities through tool declarations and resource lists, and AI agents query the descriptions through a unified interface and execute the calls, without a separate API having to be defined for every device.
