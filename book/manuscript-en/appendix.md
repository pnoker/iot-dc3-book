...
| MCP | Model Context Protocol | Open standard (Anthropic, 2024) for AI interacting with tools and data sources |
| Tool-Calling | Tool Calling | Mechanism by which an LLM operates external tools (e.g., devices) via function calls |
| RAG | Retrieval-Augmented Generation | Model answers augmented with retrieved knowledge |
| Agent | AI Agent | Multi-step intelligent agent that perceives, reasons, plans, and executes |
...
| LPWAN | Low-Power Wide-Area Network | Low-power wide-area networks (NB-IoT / LoRa, etc.) |
| RedCap | Reduced Capability | Lightweight 5G (Rel-17) for mid-tier IoT |
| TSFM | Time Series Foundation Model | Time-series foundation models (TimesFM / Chronos, etc.) for zero-shot forecasting |
| Device shadow | Device Shadow | Desired/actual device state maintained by the platform, decoupled from online status |
| Edge-cloud collaboration | Edge-Cloud Collaboration | Layered cooperation: real-time at the edge, deep compute in the cloud |
| RBAC/ABAC | Role/Attribute-Based Access Control | Access control based on roles or attributes |

## B. References

1. 3GPP TS 22.261 / 36.300 / 38.300 — Mobile system architecture and service requirements
...
8. IoT Analytics, *State of IoT* — Global IoT device scale and industry data
...
**Security**: OAuth 2.1 / JWT / X.509 certificates / TLS 1.3 / DTLS / RBAC / ABAC / multi-tenancy / prompt injection / PQC (post-quantum)

**Application scenarios**: Industrial IoT (IIoT) / digital twin / smart city / connected vehicles (V2X) / precision agriculture / blockchain + IoT / supply-chain traceability
