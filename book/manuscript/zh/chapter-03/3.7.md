...
物模型可以成为存储映射、API 合约和 UI 控件生成的输入，但只有平台实际实现相应生成器时，这些产物才会自动出现。IoT DC3 通过 `/profile` API 管理 Profile 及其位号定义，设备元数据再引用对应 Profile；这让平台知道一类设备有哪些数据点及其读写属性。新设备仍需要协议 Driver、报文解析、点位映射、单位换算、凭据和现场验收，不能从“声明 Profile ID”推导出零适配接入。设计物模型不是描述某个设备此刻的状态，而是定义一类设备的版本化能力契约；它能减少上层应用重复理解协议的工作，但兼容性必须由契约测试证明。
...
在数据互通中，物模型首先是语义契约；设备影子则是可以复用这份契约的独立运行时模式，不能把两者当成同一能力。工程上可从下面三个位置理解它们的分工。
...
**设备影子（Device Shadow）可选地拆解同步耦合。** 设备影子通常复用物模型字段维护“期望状态”和“已上报状态”：设备离线时，应用写入的是带版本与有效期的期望值；设备重新上线后再拉取、校验、执行并回报实际状态。这个模式把同步等待改为显式的异步状态机，却同时引入过期命令、冲突合并、重复执行和恢复顺序等问题。它不是物模型天然附带的能力，也不能让应用忽略设备离线；平台和 Driver 必须明确是否实现，以及版本、幂等、回执与人工接管语义。IoT DC3 当前不应被描述为提供通用 Device Shadow。
...
用一个流程可以更直观地理解这种数据互通：温度传感器上报原始报文 → Driver 按 Profile 映射解析出“温度 = 25.3℃，湿度 = 60.2%RH”及时间、质量上下文 → Data 保存位号值 → 上层应用通过统一接口读取。应用因此不必重复处理每个厂商的字节格式，但仍要理解单位、采样周期、质量与新鲜度。若另行实现设备影子，它位于命令与状态同步链路，而不是上述采集链路的必经节点。IoT DC3 通过 `/profile` 系列 REST API 管理 Profile，多个同类设备可复用定义；后续 AI 层也只能经授权 Tool 读取位号值或提交待确认 Action，不能把“统一物模型”解释成无条件读写设备。
