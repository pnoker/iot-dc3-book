...
当前消息中间件只有 RabbitMQ。Data 把位号命令与自定义命令投递到按 Driver 服务名绑定的队列；Driver 消费后执行协议操作，并把结果回执、位号值、状态和事件发送回来。平台消息链路没有 Kafka Broker；仓库新增的 `dc3-driver-kafka` 只是可选数据源驱动，不作为平台消息总线。
...
协议实现通过 `DriverProtocol` 完成真实设备读写。读取或订阅得到的数据转换为统一 `PointValue` 后，由 `DriverSenderService` 发布到 RabbitMQ。Data 的 `PointValueReceiver` 消费消息：低于批处理阈值时直接保存，高于阈值时进入 `PointValueIngestBuffer` 进程内缓冲后批量写入。Data 同时维护最新值的本地 Caffeine 缓存，并把历史数据写入 PostgreSQL Repository；持久化完成后再触发告警规则处理。

#### 位号命令与结果回执

位号读写入口位于 Data。Data 按 Driver 服务名发布命令，Driver 的 `PointCommandReceiver` 检查 `expireAt` 和 `commandId`，使用设备级锁串行化同一设备的协议操作，然后调用 `DriverReadService` 或 `DriverWriteService`。成功或失败结果经 RabbitMQ 回传 Data，消息再按执行情况 ack、reject 或 nack/requeue。Driver 队列还使用 TTL 与死信交换机控制过期和失败命令。
...
表14-2展示了上行位号值从设备到数据库的完整链路上每个组件的职责和典型风险。
...
| 消息投递 | `DriverSenderService` → RabbitMQ | 将 `PointValue` 发布到 `dc3.e.value` 等 Exchange | 使用批量 publish 提升吞吐；消息持久化（delivery mode = 2） | 生产速率 > 消费速率时队列积压；RabbitMQ 磁盘写满 |
| 异步消费 | Data 的 `PointValueReceiver` | 从 RabbitMQ 拉取消息，按阈值决策直接保存或进入 `PointValueIngestBuffer` 缓冲 | 多线程消费，每个线程独立确认；缓冲阈值可配置（例如 100 条或 1 秒） | 消费速度过慢引起消费积压；批量处理时单条异常导致整批回滚 |
| 缓存更新 | Data → Caffeine 本地缓存 | 更新设备最新位号值，供 REST 查询快速返回 | 写后立即更新并发安全 | 缓存与数据库不一致（理论窗口很小）；JVM 内存不足 |
| 持久化 | Data → PostgreSQL Repository | 将历史记录写入 `dc3_point_value` 表 | 使用 PreparedStatement 批量插入；按时间分区表 | 写入吞吐受 PostgreSQL 磁盘 IO 限制；索引膨胀 |
| 告警触发 | Data → 告警规则引擎 | 检查告警规则，生成告警记录 | 同步或异步方式，配置决定；与持久化并行执行 | 规则配置错误导致误告或漏告；告警风暴 |

#### 下行命令的异步回执

下行命令走反向异步链路：客户端经 Gateway 调用 Data 的位号命令接口，Data 将命令体发布到 RabbitMQ 的 `dc3.e.point_command` Exchange；目标 Driver 消费后执行设备操作，结果回执再经 RabbitMQ 返回 Data。客户端应通过 WebSocket 订阅或轮询 Data 提供的命令状态 API，而不是假定 HTTP 请求会一直阻塞到设备返回。
...
容量设计的原则是：先观测，后优化。在 RabbitMQ 控制台观察位号值队列的消息速率和积压情况；在 Data 的监控端点（`/actuator/metrics`）观察消费线程的 TPS 与处理延迟；在 PostgreSQL 的 `pg_stat_user_tables` 查看 `dc3_point_value` 表的写 IO 等待。只有实际压测证明单一链路成为瓶颈后，才考虑分片（例如按设备租户哈希拆分 RabbitMQ Exchange）、冷热分层（将热数据存入 Caffeine 提升读取 QPS，冷数据归档到对象存储）或引入其他存储/流处理组件。不能把 Kafka 或专用时序数据库预设成必选答案，它们在多租户隔离和内存缓存的工程模型上与 RabbitMQ + PostgreSQL 的组合存在本质差异。
...
IoT DC3 当前 Agentic Center 已实现模型配置、会话管理、Spring AI `@Tool` 调用和 Web/HTTP 对话；Gateway 的 `/mcp` 端点已实现 `initialize`、`ping`、`tools/list`、`tools/call` 等 JSON-RPC 方法，当前仅声明 Tools 能力（未含 Resources/Prompts）。项目 Compose 没有 TensorFlow Serving、训练任务、模型卷，也没有 Agentic 订阅 Data 实时位号流的默认链路。因此本节只把预测性维护作为**可选工程扩展**讨论，不能写成当前开箱即用能力。
...
部署阶段要验证 IoT DC3 当前实际组件能否在容器网络中完整启动，并跑通 Driver 业务注册、位号值上报和位号命令回执。当前官方 Compose 的基础设施是 PostgreSQL 与 RabbitMQ；平台服务包括 Web、Gateway、Auth、Manager、Data、Agentic；协议 Driver 按需启用。模板不包含 Nacos 和 TDengine，也没有模型推理容器或模型卷；2026-08 起（据项目仓库变更记录）模板新增的 `dc3-driver-kafka`、`dc3-driver-redis` 是可选数据源驱动，不是平台消息总线或缓存。
...
服务起来不算部署成功，整条链路跑通才算。下面 10 步在几分钟内验证“基础设施健康—数据上行—命令下行”三个环节，命令以 2026-08 时点的仓库 README 与官方 Quick Start 为基准；镜像 tag、端口与路径随版本演进，以仓库 README/Compose 为准，标注“示意”的细节为教学示意，不要照抄。
...
预期：`dc3-postgres`、`dc3-rabbitmq` 状态 Up；能列出 auth schema 的表。宿主机映射端口以 `.env` 为准（当前 Quick Start 为 PostgreSQL 35432、RabbitMQ AMQP 35672，容器内仍是 5432/5672）。
...
预期：返回 5 分钟有效的 salt；再用 `/api/v3/auth/token/generate`（携带 salt 与按规则哈希后的密码，哈希规则以官方 Quick Start 为准）换取 12 小时有效的 Token。此后所有请求统一携带 `X-Auth-Tenant`、`X-Auth-Login`、`X-Auth-Token` 三个头。Gateway 是唯一外部 HTTP 入口（8000 端口），Auth/Manager/Data 的直连端口只用于调试。
...
预期：返回随栈启动的 Driver 列表——Driver 能出现在这里，说明 14.2.3 所述的 gRPC 业务注册已经成功。随后按官方 Quick Start 的顺序创建 profile、位号（例如 Temperature，FLOAT、READ_WRITE）和绑定该 Driver 的设备，记录 deviceId 与 pointId。MQTT 场景确认 `dc3-driver-mqtt` 在列表中且 Compose 已启用该服务；若暂无外部 MQTT Broker，可先用自带的 virtual Driver 走完整个序列。

**第 6 步：以设备身份上报一条位号值。**

```bash
mosquitto_pub -h <broker-host> -p 1883 \
  -t '<由 dc3-driver-mqtt 定义的设备上报 Topic>' \
  -m '{"deviceCode":"<device>","values":{"Temperature":25.6}}'   # Topic 与字段结构示意，以驱动实现为准
```

预期：命令正常返回即发布成功。注意 14.2.4 的事实边界：MQTT 的 Topic 与 Payload 结构由具体 Driver 定义，上面只是结构示意。MQTT Driver 需要一个可达的 Broker，地址与端口在其驱动配置中设定。
...
预期：返回该位号的最新记录（rawValue、numValue、createTime 等字段）——说明“Driver → RabbitMQ → Data → PostgreSQL”上行链路贯通。
...
podman logs dc3-driver-mqtt --tail 50     # 或实际启用的驱动容器
```

预期：Data 日志出现位号值消费与保存记录，Driver 日志出现注册与读写执行记录；再对照 RabbitMQ 管理台确认相关队列无积压、无死信。至此，上行与下行两条链路都有可检查的证据。
...
| 数据上报 | 用 `mosquitto_pub` 以设备身份发布一条报文（见上文 10 步序列第 6 步）；Modbus 场景用 diagslave 等从站模拟器保持寄存器可读 | 位号值经 RabbitMQ 进入 Data 并写入 PostgreSQL |
| 命令下发 | 调用 Data 位号命令接口 | RabbitMQ 投递到目标 Driver，结果回执返回 Data |
