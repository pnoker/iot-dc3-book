...
当前消息中间件只有 RabbitMQ。Data 把点位命令与自定义命令投递到按 Driver 服务名绑定的队列；Driver 消费后执行协议操作，并把结果回执、位号值、状态和事件发送回来。平台消息链路没有 Kafka Broker；仓库新增的 `dc3-driver-kafka` 只是可选数据源驱动，不作为平台消息总线。
...
下行命令走反向异步链路：客户端经 Gateway 调用 Data 的点位命令接口，Data 将命令体发布到 RabbitMQ 的 `dc3.e.point_command` Exchange；目标 Driver 消费后执行设备操作，结果回执再经 RabbitMQ 返回 Data。客户端应通过 WebSocket 订阅或轮询 Data 提供的命令状态 API，而不是假定 HTTP 请求会一直阻塞到设备返回。
...
部署阶段要验证 IoT DC3 当前实际组件能否在容器网络中完整启动，并跑通 Driver 业务注册、位号值上报和点位命令回执。当前官方 Compose 的基础设施是 PostgreSQL 与 RabbitMQ；平台服务包括 Web、Gateway、Auth、Manager、Data、Agentic；协议 Driver 按需启用。模板不包含 Nacos 和 TDengine，也没有模型推理容器或模型卷；2026-08 起模板新增的 `dc3-driver-kafka`、`dc3-driver-redis` 是可选数据源驱动，不是平台消息总线或缓存。
