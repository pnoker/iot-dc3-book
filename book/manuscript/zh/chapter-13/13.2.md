...
**密钥与设备绑定：物理锚定。** 信任根必须从代码迁移到硬件。具体做法是，每台设备在出厂阶段，于板载**安全飞地**（Secure Enclave）中生成一对非对称密钥。这块物理隔离的安全元件，密钥一旦生成就不可导出，只允许通过接口调用签名或解密操作。不少物联网硬件厂商选用具备低成本安全芯片的器件（例如 Microchip ATECC608A 一类工业级安全芯片）来存储椭圆曲线密钥并执行ECDSA签名。主控MCU只持有公钥，从未触碰私钥。即使攻击者通过物理手段提取MCU固件，也无法获取签名私钥。更高安全等级的场景——如智能电表、工业控制器这类需合规认证的设备——可使用可信平台模块（Trusted Platform Module, TPM）。设备据此构造自己的DID：常见做法是用公钥的SHA-256哈希作为DID中的`method-specific-id`。一台设备的DID因此在物理层面绑定到了它的硬件密钥，而不是一个可被复制或转移的软件文件。
