...
两个关键点。第一，`@Tool` 的名称、`description`、参数 Schema 和运行时上下文共同影响模型如何选择函数；描述越精确，越容易减少歧义，但不能保证不误调用。第二，`@ToolParam` 的 `description` 可以说明业务含义、单位与示例，参数类型和取值范围仍应由 Schema 与服务端校验明确约束；不要依赖模型把含混的 1/0 自行猜成布尔值。
...
漏传 `conversationId` 是多轮对话最常见的接线错误：advisor 拿不到会话标识，历史注入为空，模型表现为“失忆”——答非所问或反复追问已经给过的信息。排查这类问题时，应先检查请求中 advisor 参数是否携带了会话 ID，再怀疑模型本身。
