...
**治理平面**已经覆盖关键写入链路。`ToolContext` 提供租户、用户和会话上下文；位号写 Tool 创建有效期有限的 `PENDING` Action，用户确认后才由 `ActionService` 进入实际命令链路。MCP 入口有独立的 OAuth、连接授权、工具白名单和确认状态，不能与 Agentic Center 内部 Action 简化成同一个拦截器。现有日志、消息、Action 和命令记录为审计提供了基础，但还缺少统一的 `run_id`、任务状态机、步骤级 Trace、租约、恢复与补偿语义。
