...
**工程价值**：MQTT在边缘侧拥有不可替代的生态位。电池供电设备、高延迟网络、偶发断连场景，MQTT通常是长期最合理的默认选择。QoS等级按业务容忍度选择即可：QoS 0用于可丢失的高速采样（如温度数据），QoS 1用于必须送达但允许重复的指令（如远程配置），QoS 2的多轮确认开销在物联网场景中鲜有用武之地，实际使用较少。
