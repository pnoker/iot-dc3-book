...
MCP（Model Context Protocol，模型上下文协议）正是在这一背景下产生的。它不是设备协议，而是一个专门面向 AI 应用与外部工具、资源和知识库交互的会话协议。

在下文展开之前，先划清两类陈述的边界：关于 IoT DC3 的部分，引用的是项目文档中 MCP 实现的公开设计方向，属于项目现状的事实描述；关于该类协议在物联网中的应用方式与价值边界，则是作者的工程判断，并非正式标准的最终定义，部分细节以示例方式呈现。
...
- **Tools**：可由模型请求触发的可执行动作。每个 Tool 声明自己的输入 Schema，描述参数名称、类型、约束和是否必填。AI 模型不直接执行工具，而是生成调用请求，由 MCP 服务器校验权限、执行操作并返回结果。IoT DC3 采用“一个网关 + 四个中心服务”的架构：Auth 中心负责认证授权，Manager 中心管理设备与配置元数据，Data 中心承载数据管道，Agentic Center 提供 AI 智能体能力。其 MCP Server 通过聚合这四个中心服务的 OpenAPI 自动生成工具目录。
- **Prompts**：可复用的、参数化的提示模板，允许服务端指导模型“如何理解这个领域的资源”。

IoT DC3 的 MCP 网关使用 JSON-RPC 2.0 交换消息。客户端和服务端通过能力协商完成初始化，声明支持的协议版本和能力列表。服务端通过 `tools/list` 返回可用工具列表，客户端通过 `tools/call` 触发工具执行。整个交互是有状态的——每次调用携带一个 request ID，响应与请求在 session 内关联，支持超时和错误处理。通信走基于 Token 的安全认证（OAuth 2.1 授权机制见第 8 章 8.5.3 节），结合工具白名单与风险分级，确保每次调用都在既定安全策略下进行。

图 9-9 展示了 MCP 协议在 IoT 场景中一次典型的交互序列，涵盖资源发现、工具调用和状态反馈。
...
MCP 的消息载体采用 JSON‑RPC 2.0。这一选择并非技术上的最优解——如果追求 payload 体积最小化，CBOR 或 protobuf 更合适——而是为了降低 AI 应用侧的接入门槛：任何支持 JSON 序列化的语言都能直接处理消息，无需引入额外序列化库。传输方式上，MCP 官方规范（最新为 2026-07-28 版）只定义了两种：stdio 与 Streamable HTTP——前者把 MCP 服务器作为本地子进程拉起，经标准输入输出交换消息；后者面向远程部署，客户端向服务端的一个 HTTP 端点发请求，服务端可按需流式返回增量结果。早期规范中的 HTTP+SSE 传输自 2025-03 版起已废弃，由 Streamable HTTP 取代；WebSocket 从未列入官方传输方式，IoT DC3 的 `POST /mcp` 即为 Streamable HTTP 形态。规范本身仍在快速演进：2026-07-28 版引入了 Tasks 扩展，为长时间运行的服务端任务提供创建、查询与取消的状态语义；官方 Registry（registry.modelcontextprotocol.io）也已上线，作为服务器目录的权威分发渠道。第 7 章 7.1.5 节从 IoT DC3 的实现边界讨论过这些机制，本节关注的是协议层次与标准状态本身。但需要厘清边界：MCP 不定义设备侧的信封、帧头或 payload 格式，那些是 MQTT、CoAP 或 HTTP 的分内之事。MCP 只解决一个问题：AI 应用如何发现并调用外部能力，外部能力又如何描述自己。
...
`protocolVersion` 的值在具体实现中各不相同，这里仅为示例。能力协商不是一次性授权凭证——每次新会话都应重新校验。服务器可能更新了可用的 tools 列表或资源路径，client 不应持有过期清单并在多个会话间复用。
...
MCP Server收到请求后，通过内部处理器调用IoT平台API，平台再通过MQTT下发真实指令。下面的Python代码模拟了从Agent到Server再到平台和设备的完整消息流——Streamable HTTP 端点与会话管理从略，只聚焦核心消息处理与状态变化：
