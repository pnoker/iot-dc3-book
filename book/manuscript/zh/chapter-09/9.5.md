...
- **Tools**：可由模型请求触发的可执行动作。每个 Tool 声明自己的输入 Schema，描述参数名称、类型、约束和是否必填。AI 模型不直接执行工具，而是生成调用请求，由 MCP 服务器校验权限、执行操作并返回结果。IoT DC3 采用“一个网关 + 四个中心服务”的架构：Auth 中心负责认证授权，Manager 中心管理设备与配置元数据，Data 中心承载数据管道，Agentic 中心提供 AI 智能体能力。其 MCP Server 通过聚合这四个中心服务的 OpenAPI 自动生成工具目录。
- **Prompts**：可复用的、参数化的提示模板，允许服务端指导模型“如何理解这个领域的资源”。

IoT DC3 的 MCP 网关使用 JSON-RPC 2.0 交换消息。客户端和服务端通过能力协商完成初始化，声明支持的协议版本和能力列表。服务端通过 `tools/list` 返回可用工具列表，客户端通过 `tools/call` 触发工具执行。整个交互是有状态的——每次调用携带一个 request ID，响应与请求在 session 内关联，支持超时和错误处理。通信走基于 Token 的安全认证（该项目采用 OAuth 2.1 机制，详见第8章8.5节），结合工具白名单与风险分级，确保每次调用都在既定安全策略下进行。
