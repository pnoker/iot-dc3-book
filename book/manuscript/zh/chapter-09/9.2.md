...
MQTT 的消息路由依赖 Broker。发布者向主题发送消息，Broker 查找匹配订阅并转发。发布者与订阅者无需互知 IP，拓扑和消费节奏可以相对独立；但时间解耦取决于 QoS、持久会话、订阅状态、消息过期和 Broker 存储。没有这些条件，订阅者离线期间的消息仍可能丢失，因此不能概括为“完全解耦”。
...
- **QoS 1（至少一次）**：发送方保留 PUBLISH 并等待 PUBACK，未确认时按实现策略重发，因此协议交付可能重复。PUBACK 只确认相邻 MQTT 端已接收该 PUBLISH；跨 Broker、数据库和物理设备的交付仍取决于会话、持久化与后续处理。适用场景包括需要确认且能以业务键去重的状态或事件；设备命令还需到期时间、幂等和执行回执。
...
选型时要同时判断可否丢失、可否重复、断线重连语义和业务幂等。多数项目以 QoS 1 配合业务键、状态机和去重表；只有在协议交付范围确有必要时才使用 QoS 2。无论选择哪一级，跨 Broker、数据库和物理设备的“业务恰好一次”都必须由应用协议另行建立并验证。急停、联锁等人身安全功能应由本地安全系统完成，不能依赖 MQTT QoS 作为唯一保障。
...
每个传感器是一个 MQTT 客户端，连接 Broker 后以固定间隔向主题发布数据。场景中选用 QoS 1：客户端在收到 PUBACK 前保留并可能重发 PUBLISH，从而降低瞬时链路丢包造成的丢失风险，代价是可能重复。它不证明数据已经入库或被业务消费，因此后续仍需幂等键、落库确认和监控。
...
代码的关键点：用 `+` 通配符订阅所有传感器的温湿度主题，平台无需预先枚举传感器 ID；湿度超过阈值时，向告警主题发布 QoS 1 消息，并在载荷中携带 `dedup_key` 供订阅端去重。设计目标是降低丢失风险并阻止重复投递变成重复通知，但这段示例没有实现持久化发件箱、消费确认或端到端事务，不能宣称“不丢不重”。遗嘱消息也只在 Broker 判定异常断开后发布，设备状态仍需结合心跳时间与业务观测。
...
| 告警推送 | 1 + 幂等键去重 | false | 降低链路丢失风险，重复投递按告警唯一键去重；端到端可靠还需持久化、消费确认与补偿 |
| 遗嘱状态 | 1 | true | 新订阅者立即获得设备状态 |

这个案例展示了 MQTT 在轻量级物联场景中的基本工作流：设备通过长连接定期发布，平台用通配符订阅统一接收，告警以 QoS 1 配合幂等键降低丢失与重复副作用，遗嘱消息辅助识别异常断开。示例刻意省略了持久化发件箱、消费确认、分片和事务，因此适合解释协议语义，不是端到端可靠性的生产模板。
