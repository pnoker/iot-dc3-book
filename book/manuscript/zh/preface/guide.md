...
想把书中的系统亲手跑起来，准备工作并不重：一台装好 Docker（或 Podman）与 Compose 的开发机、JDK 21，再备一个 MQTT 客户端（如 mosquitto_pub）用于模拟设备上报。第 14 章的实战部分给出了可直接复制的 10 步冒烟命令序列与故障排查表，按顺序执行，就能验证从设备接入、数据入库到智能体调用的完整链路。
