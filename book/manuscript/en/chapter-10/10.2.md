...
The Modbus protocol defines a register address space. Whether the physical substrate is a PLC's memory area or a sensor's memory, it is logically abstracted into four kinds of data objects (see Table 10.2.1-1). Understanding this model is the foundation of driver configuration.

**Table 10.2.1-1 Common Modbus function codes**
...
The CRC check uses CRC-16 (Modbus polynomial 0x8005), safeguarding data integrity on the serial link. Modbus TCP drops the CRC and adds a transaction identifier to the frame; the protocol's data structure itself is unchanged, and TCP mode runs over port 502.
...
In the four-layer IoT architecture, OPC UA belongs to the network layer (fieldbus), connecting downward to PLCs/controllers and handing data upward to the data platform. Unlike Modbus, which uses only TCP port 502, OPC UA uses the `opc.tcp://` protocol (port 4840 by default) and builds in session management, secure channels, and data encryption.
