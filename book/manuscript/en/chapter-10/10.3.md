...
| Write performance | Million-point-per-second class on a single node (illustrative order of magnitude, not a benchmark) | Near-native PG write speed through partition optimization | High-throughput optimization via columnar storage + pre-computation |
