...
**Second, embodied intelligence and Physical AI move from pilot to volume production.** 2027–2028 is widely seen as the commercialization inflection point of the industrial humanoid robot (an industry projection, not a verified fact): several leading manufacturers have announced production-line deployment timelines, and the China Academy of Information and Communications Technology (CAICT) has listed "IoT-embodied intelligence" as a main direction for extending IoT into the physical world. Embodied intelligence emphasizes the "perceive — understand — execute" closed loop, and its capability ceiling depends on the scale of real-world data — the industry expects that once high-quality training data accumulates to the order of tens of millions to hundreds of millions of hours, embodied intelligence may see a breakthrough like the large language models' "GPT-3.5 moment" (these figures are industry projections and remain unverified). What this path rests on is precisely the engineering foundation this book has stressed throughout: low-latency inference at the edge, a unified thing model, and an execution chain that can write back.
