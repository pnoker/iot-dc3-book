...
- **Smart lighting module**: LED lamp heads paired with DALI-protocol drivers, supporting stepless dimming (source: this book's example; the dimming range serves only to illustrate the control logic). The finer the dimming step, the smoother the dynamic dimming (light brightens as a vehicle approaches, dims once it has passed) and the less interference with camera captures. Lighting commands must get a fast response locally on the edge box.
...
- **Environmental sensor module** (temperature/humidity, PM2.5/PM10, noise): sampling period of 1–5 minutes (example), each message under 1 KB (example). Requirements on timestamp synchronization are high — citywide air-quality contours require cross-sectional data captured at the same instant.
...
The edge computing box is the pole's "brain." Different sensors use different physical protocols (lighting over DALI, cameras over RTSP, environmental sensors over RS-485 Modbus, charging points over the CAN bus). Under the example, the hardware configuration is a quad-core ARM processor plus one NPU, 4 GB of memory, and 32 GB of eMMC storage. The NPU runs a pruned, INT8-quantized YOLOv5 variant (about 7 M parameters in this scenario, with single-frame inference taking tens of milliseconds). The video stream is not processed at full frame rate; the frame rate is reduced (for example to 12 fps) to meet traffic-counting needs. Power is the root of the trade-offs: assume the pole's power-distribution capacity is capped at 500 W and LED lighting consumes 80–150 W, leaving only a limited margin for the edge computing box — on the order of 30 W (example configuration). An NPU-plus-ARM-core combination usually falls within that budget.
...
The core idea of the configuration is "termination at the edge": high-bandwidth devices such as cameras are digested locally and output only structured messages; lighting commands are low in volume but need low latency; charging points involve transactions and must be encrypted separately. One engineering check — verify whether the actual per-pole uplink bandwidth under the example stays within a reasonable range — and if it is exceeded, add data compression or secondary aggregation inside the edge box.
...
| Inconsistent data standards across departments | Event routing fails or information is lost | Before go-live, uniformly adopt the national emergency-management data exchange standard (such as GB/T 35555-2017) and build a field-mapping cross-reference table |
