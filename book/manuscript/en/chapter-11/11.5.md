...
This chapter began with three engineering questions: how to validate latency, reliability, and coverage boundaries for V2X communication at speed; how an edge–cloud architecture can carry a city-scale event stream; and where AI may intervene to move from after-the-fact alarms toward evidence-backed prediction and intervention candidates. The three constrain one another: business deadlines affect edge placement, event rate and message size determine middleware capacity, and model processing remains bounded by tail latency, data quality, and safety policy. The chapter offers candidates to validate, not universal answers. PC5 and Uu are complementary communication modes with different ranges and dependencies; whether they form redundancy must be demonstrated by the terminal, network, and application design. Kafka partitions and edge pre-aggregation need load tests against the target event model. Deep reinforcement learning for signal control requires offline evaluation, simulation, constraint policies, and progressive field validation. What transfers is the trade-off method: where compute belongs, where data is filtered, how quickly a model may make which class of judgment, and who takes over on failure.
