...
3. Attach auxiliary features to each sample: time of day (the minute within the day, normalized to [0,1]), day of the week (one-hot encoded), and a holiday flag (binary variable).
4. Apply Z-score standardization across all samples to remove differences in scale.

The final input tensor then has shape `(num_samples, 96, 3)`, where the 3 channels are the flow value, the time-of-day code (the normalized scalar), and the day-of-week code (dimension 1 of the one-hot vector, with the rest omitted).

@[fig-11-09]

#### LSTM Principles and Engineering Implementation

An LSTM manages what it remembers and forgets through three gated units — the forget gate, input gate, and output gate — avoiding the vanishing/exploding gradients of long-sequence training. In traffic-flow scenarios an LSTM can retain the flow pattern from the same period a week ago as hidden state, which linear models such as ARIMA cannot do.

The following snippet implements training of the above model in Keras, with the data handling assumed:
...
After sufficient training (in the example), the agent significantly outperforms fixed-time plans across different traffic volumes. The magnitude of the improvement varies with intersection topology and flow.
...
Streetlight optimization is a typical entry point for energy saving in a smart city. Traditional strategies mostly switch all lamps on and off by schedule — in the late night, when street traffic is very low, an entire street still runs at full power. The goal of AI dimming is to adjust each lamp's brightness dynamically from real-time pedestrian and vehicle flow without lowering public safety. Everything below in this section is a case under the example: the data and parameters serve to illustrate principles and methodology and do not represent actual project results.

#### Deep Q-Network Dimming Model

When streetlight dimming is placed in a reinforcement-learning framework, each lamp is abstracted as an independent agent. The state, action, and reward designs below are all from the example.
...
In the example, evaluation typically focuses on three metrics (source: this book's example; the metrics illustrate the control trade-offs): energy saved, illuminance compliance, and recovery response after a burst of traffic. The following compares power curves for a secondary road.
