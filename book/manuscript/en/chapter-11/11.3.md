...
The following table summarizes key performance indicators and engineering rules of thumb for million-scale access scenarios. All values in the table are ranges based on typical examples.
...
|--------|--------------|----------------|
| Concurrent connections | 200,000 lamp poles + 100,000 RSUs + 700,000 in-vehicle terminals (example) | Keep a single MQTT broker's connection count in the tens of thousands; beyond that, scale horizontally, combined with session persistence |
...
| Authentication shock | Tens of thousands of simultaneous authentications during unified device startup (example) | Cache tokens in Redis to avoid querying the database on every request |
...
Table 11-5-1, suggested template:
...
Take an example: in a new district's smart-traffic system, several intersection RSUs and multiple aggregation edge nodes are deployed (**an example case**).
...
When different task types are handled at different tiers, end-to-end latency, network bandwidth consumption, and computing cost differ significantly. The table below compares them; the figures are based on typical engineering ranges (a comparison table):
...
**Table 11-1:** Latency, bandwidth, and cost comparison across tiers (example data, based on typical engineering ranges).
