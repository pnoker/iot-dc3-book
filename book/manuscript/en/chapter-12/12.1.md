...
Where grid power is unavailable and solar irradiation is suitable, photovoltaic generation plus a battery is a common standalone-power candidate for agricultural sites; it is not the default optimum for every farm. Canopy shade, high-latitude winters, consecutive overcast days, low-temperature charging, and maintenance access may instead favor grid power, primary batteries, or a hybrid supply.
...
As data travels from node to cloud, transport choice directly affects reliability and power consumption. MQTT (Message Queuing Telemetry Transport) is a common option for agricultural backhaul, but its position must be stated correctly: it runs over a network that provides an ordered, reliable, bidirectional connection and is typically used on the gateway-to-cloud IP link, not directly on the LoRa air interface. The node-to-gateway hop uses proprietary LoRa frames or LoRaWAN MAC frames; a gateway or network server recovers the radio payload and an application then bridges the business data to MQTT. MQTT has a small fixed header and a publish/subscribe model. Whether a disconnected client resumes its session and recovers unacknowledged messages depends on the Client Identifier, Clean Start, Session Expiry, QoS, and broker state; it is not simply "seamless recovery."
