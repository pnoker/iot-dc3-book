...
For nodes that reach the cloud directly over NB-IoT or 4G (such as the image-capture node later in this chapter), the MQTT client runs on the node itself; CoAP (Constrained Application Protocol) is its lightweight alternative, but its message reliability and firewall traversal generally fall short of MQTT — unless the node's resources cannot even fit an MQTT client library, prefer MQTT. The trade-off rules among the three quality-of-service levels QoS 0/1/2 were laid out in Section 9.2, and agriculture applies their conclusions directly: periodically sampled soil temperature uses QoS 0, while threshold alarms use QoS 1 to ensure packet loss never swallows them.
