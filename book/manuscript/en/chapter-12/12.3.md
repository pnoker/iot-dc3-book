...
Table 12-3 pulls these requirements into one clear comparison sheet for direct reference during the technology selection that follows.

**Table 12-3 Core requirements that the agricultural IoT scenario places on LPWAN communication technology**
...
Table 12-2 compares them side by side across five aspects: frequency band, rate, link budget, network architecture, and cost structure. The data are based on the technical specifications published by each technology alliance, with some figures being industry consensus or ranges (actual values fluctuate with configuration and purchase volume). One point needs stating: the direct evidence available in the reference pack is limited — LoRa operates in the unlicensed Sub‑GHz band, LoRaWAN can be built, customized, and managed by anyone, and the LoRa Alliance has more than 150 member companies; NB‑IoT requires a SIM card and an operator tariff. The remaining parameters come mainly from publicly released technical documents, without independent sources in the reference pack.
...
| **Typical uplink rate** | As low as 0.3 kbps, as high as 50 kbps (depending on the spreading factor) | Theoretically up to several hundred kbps uplink; in practice limited by coverage and scheduling | Extremely low (typically about 100 bps) |
...
| **Ecosystem openness** | LoRa Alliance 150+ members; data sovereignty can be kept in-house | Closed to operators; data tied to the SIM card | Closed ecosystem; a single chip supplier |

**Table 12‑2 Core parameter comparison of LoRa / NB‑IoT / Sigfox** (Sigfox has left the mainstream and appears in the table only as a historical route reference)
...
The three differ enormously in rate. LoRa's physical-layer rate spans 0.3 ~ 50 kbps, and its payload ranges from 242 bytes (SF7) down to 51 bytes (SF12) with the spreading factor; NB‑IoT can theoretically reach several hundred kbps uplink, enough to support FOTA (Firmware Over‑The‑Air upgrades); Sigfox's uplink is fixed at an extremely low rate (about 100 bps) with a 12-byte payload — it cannot even send a complete GPS coordinate, only predefined enumerated status codes. For unlicensed-band options, remote upgrade is all but infeasible — run a hypothetical calculation: at LoRa's fastest rate, downloading 2 MB of firmware over the downlink at 50 bps takes about 90 hours (based on the physical layer and a typical frame structure), and the node's battery simply cannot sustain that drain.
...
**Table 12‑3 Battery life estimates at different reporting periods**
