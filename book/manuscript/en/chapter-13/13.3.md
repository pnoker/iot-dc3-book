...
**Secure Multi-Party Computation (MPC)** lets several parties compute a function under an agreed security model while limiting disclosure beyond the inputs, outputs, and information permitted by the protocol. Common constructions include secret sharing and garbled circuits. MPC is not a single trust model: semi-honest versus malicious adversaries, honest versus dishonest majority, fairness or abort, setup assumptions, and implementation side channels all change the guarantee. Performance cannot be summarized as "orders of magnitude slower" either; rounds, bandwidth, and computation depend on the protocol, function, participant count, and network. In IoT, MPC is often better separated from control and used for joint statistics, risk analysis, or pricing. Any low-latency use must be justified by the threat model and a target-deployment benchmark, and it cannot replace local safety interlocks.
...
|------|----------|----------|--------|-------------|----------|
| Federated learning | Raw training data remains with participants | Depends on rounds, model, and network | Established engineering deployments | Cross-enterprise joint failure-prediction models | Updates and outputs may still leak information; needs secure aggregation, DP, and governance |
| Differential privacy | Bounds one record's influence on output under a privacy budget | Depends on mechanism and utility target | Mature in statistical use cases | Device population profiling, statistical reports | Precision loss; unsuitable for single-point control |
| Secure multi-party computation (MPC) | Parties' inputs and intermediate state | Protocol- and network-dependent, often above plaintext | Usable in specific scenarios | Multi-party joint risk control, joint pricing | Complex trust model and implementation; low-latency capability requires measurement |
| Trusted execution environment (TEE) | Code and data inside a defined execution boundary | Workload- and I/O-dependent | Commercial implementations exist | Model-inference protection at edge nodes | Depends on hardware and attestation chain; side-channel, rollback, and availability risks |
| Homomorphic encryption (HE) | Plaintext during selected computations | Operator- and parameter-dependent; FHE is often costly | Practical for selected operators | Encrypted aggregation, constrained encrypted inference | Ciphertext expansion and parameter/operator constraints; benchmark required |

Selection begins with four questions. **First, what is the protection goal**: bounding an individual's influence on a statistical output, keeping raw training data local, protecting multiple parties' inputs, or isolating runtime code? Different tools address different goals; "data residency" alone is not a design. **Second, what is the adversary and trust model**: participant count, tolerated collusion, abort behavior, and trust in hardware or a coordinator all matter. **Third, what are the latency and failure semantics**: privacy mechanisms do not belong in local safety interlocks, and analytical or optimization paths still need measurements for tail latency, timeout, retry, and degradation. **Fourth, what are the compute and network budgets**: benchmark target data and parameters on the MCU, gateway, and cloud before choosing local execution, edge proxying, or offline batching. A technology name does not predict performance.

One common confusion deserves clarification: privacy-enhancing computation and verifiable records solve different problems. They neither substitute for one another nor always need to be combined. A ledger, signature, or ZKP can make a submission verifiable under stated identity, key, and time assumptions, but it cannot prove that the sensor reading was true at the source. Privacy-enhancing computation can limit some disclosure during collaboration, but it neither grants data-use rights automatically nor guarantees that outputs reveal nothing. Combine the two only when cross-organizational collaboration needs both witnessed records and constrained computation.

Returning to this chapter's main thread: in cross-organizational AI-agent collaboration, contracts, purpose limitations, and access control determine whether data may be used; privacy-enhancing computation constrains information exposure during computation; and DIDs, credentials, signatures, or ledgers help verify identity claims and records under their respective assumptions. None of these changes the deterministic constraints on AI actions in the physical world. Permission, confirmation, and audit (Chapters 7 and 8) remain mandatory. Data privacy is not behavioral safety, and cryptographic verifiability is not physical truth.
