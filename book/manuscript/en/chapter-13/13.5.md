...
### 13.5.3 A Case Study in Smart-Contract-Assisted Authorization and Audit

When a theoretical framework meets an engineering scenario, one recurring question is: “how can an AI prediction become an auditable candidate action without crossing the field-control boundary?” This smart-irrigation example shows an oracle submitting prediction evidence with provenance and version, a permissioned ledger or smart contract recording the authorization outcome, and a deterministic Workflow driving equipment only after policy, confirmation, and interlock checks and then writing audit events back. The ledger supports authorization and accountability; it does not replace sensor validation, controllers, or safety interlocks.
...
3.  **Condition-triggered contract proposal**: the smart contract, deployed on a permissioned chain, produces an irrigation proposal only when the oracle identity, model version, input freshness, and confidence threshold satisfy policy, and it records the policy version. The contract neither operates the solenoid valve nor grants final authorization from one model result. The edge execution service still validates zone permissions, soil and weather rules, device interlocks, duration limits, and human-confirmation state. Only after all conditions pass does a deterministic Workflow create a signed command containing the target zone, irrigation duration, and deadline.
