...
Seen from the smart-contract side, the data verification and traceability mechanism is one comparison function plus a set of event logs. Placed inside the whole blockchain-IoT system, these two functions form the anchor of the chain of trust: **the on-chain hash guarantees the data cannot be tampered with, and the on-chain events guarantee operations cannot be repudiated.** Together, every bit of data a device produces acquires a digital dossier that is traceable, auditable, and verifiable. That said, what this mechanism proves is only that "the data has not been altered"; it cannot yet answer another class of question — whether this device is qualified to make a given claim, such as whether its calibration certificate is still valid or who issued the quality-inspection conclusion. That is precisely what the verifiable credentials introduced in the next subsection are for.
...
The division of labor between VCs and DIDs must be kept straight: the DID document answers "who you are and which key you sign with," resolved through the on-chain registry of Section 13.2.1; the VC carries verifiable claims about "who granted you what qualification," held by the device and presented on demand. Only with the two stacked together does a device possess both "a resolvable identity" and "verifiable claims." On the standardization side, the W3C published VC Data Model 2.0 as an official Recommendation in May 2025, while DID Core 1.1 is at the Candidate Recommendation stage; together they form the data foundation for cross-organizational mutual recognition of device identity. The EU's eIDAS 2.0 and the EUDI digital identity wallet target people and organizations, but their "issue — hold — verify" regulatory framework is extending toward device and machine identity — an external push that the industrial IoT can borrow directly.
