...
| Device access | Independent protocol Drivers; module count and capability depend on the evaluated code snapshot | Basic protocols via integration layer | Device SDK, edge gateway |
...
| Selection focus | Independent Agentic Center, protocol Drivers, and a governed command path | Device management, visualization, and rules | Edge SDK and Kubernetes deployment |

Selection advice: if the architecture needs an independently deployed intelligence layer and you want to validate a governed loop over existing device metadata, point values, and command paths, IoT DC3 belongs on the candidate list—but Tool registration, write confirmation, and command receipts must be checked against a named code revision. If the priority is device management, visualization, and rule triggering, evaluate ThingsBoard separately. If the team already operates Kubernetes and emphasizes an edge SDK and clustered deployment, evaluate Kaa separately. Labels in this table do not replace a proof of concept: compare access, governance, observability, and recovery with the same device model, load, and fault script.
