...
| Device access | 36 driver modules (including a small number of data-source/virtual drivers), via Gateway | Basic protocols via integration layer | Device SDK, edge gateway |
...
The network layer focuses on three low-power wide-area networks (LPWANs): LoRaWAN, NB-IoT, and 5G URLLC; also take note of 5G RedCap from Release 17 — a lightweight 5G profile standardized in 2022 and commercially rolling out from 2023, aimed at mid-rate IoT scenarios such as wearables and video backhaul. There is no need to memorize channel parameters, but you should be able to judge the selection across dimensions such as coverage radius, power consumption, and data volume.
...
Start from OpenAI's Function Calling documentation to understand how the LLM parses function names and parameter signatures out of natural language. Then look at LangChain's Tool abstraction — it standardizes tool registration, invocation, and result return into an interface. The Spring AI `@Tool` annotation DC3 uses is essentially the same mechanism moved into the Java ecosystem. On security, DC3's MCP integration is a ready-made, operable reference: the OAuth 2.0 authorization-code flow (industry practice has widely adopted OAuth 2.1 draft provisions such as mandatory PKCE), a tool whitelist, and risk grading — three layers of constraint. Finally, read the MCP specification (now hosted by the Linux Foundation) to work out the protocol layer's independent authorization handshake.
...
In this step you get hands-on with three open-source projects, in order. First deploy IoT DC3 (github.com/iot-dc3/dc3): run it on a single machine with docker-compose and manually walk the full chain of device registration → driver configuration → point mapping → rule engine → Agentic Center tool calling, living through a complete new data loop. Next, try ThingsBoard's (github.com/thingsboard/thingsboard) visual drag-and-drop rule engine, compare it with DC3's code-driven approach, and sort out which logic drag-and-drop can handle and which must be handed to the LLM. Finally, look at Apache StreamPipes (github.com/apache/streampipes) for stream processing of industrial data pipelines, as a reference implementation for platform-layer data cleansing and preprocessing.
