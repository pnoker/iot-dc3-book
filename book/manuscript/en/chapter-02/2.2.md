...
**The decide stage** turns state judgments into candidate actions. A traditional rule engine handles propositions such as “IF attribute value > threshold THEN trigger action,” which suits operating conditions with explicit thresholds and fixed scenarios. In a complex system of coupled variables, however, one threshold is not enough: air-conditioning efficiency must weigh outdoor temperature, occupancy, electricity-price periods, and start-stop energy costs together. The decide stage proposes candidate sequences within those constraints. Rule engines handle known cases inside deterministic boundaries; in nondeterministic cases, an AI model, such as an LLM integrated through IoT DC3's Agentic Center, may generate the next-step proposal. The output should be a structured candidate Action containing the target device, parameters, evidence, risk class, priority, and expiry. It becomes an executable command only after authorization, policy, conflict, and required confirmation checks pass.

**The execute stage** is the critical step that carries approved commands back into the physical world, covering parameter validation, risk policy, queue delivery, protocol-driver adaptation, and receipt confirmation. Once a command is approved, the platform locates the protocol Driver for the target device, translates an abstract operation such as "set temperature to 25.5 °C" into a Modbus register write or a PLC message, and delivers it over the appropriate communication link. The device receipt and post-execution readback are then used to determine whether the loop actually closed. This is where network latency, protocol differences, offline devices, duplicate delivery, and conflicting commands surface, so execution needs retries, idempotency, timeouts, conflict detection, and human takeover. In the IoT DC3 code boundary used by this book, the Manager Center maintains business metadata such as devices and accepts Driver business registration; point commands enter through Data, travel to the Driver over RabbitMQ, and return receipts over the messaging path. End-to-end latency and reliability must be measured in the target deployment; neither "millisecond-level coordination" nor guaranteed delivery follows from the architecture diagram alone.
...
**The decide stage** — under the building's energy-management policy, the planning component generates two structured candidate Actions: (1) turn on the southeast conference room's air conditioning at 24 °C and medium fan speed; (2) raise the northwest office area's setpoint by 2 °C. Each candidate carries its evidence, applicable period, risk class, and a reevaluation condition 30 minutes later.

**The execute stage** — a policy service checks authorization, comfort bounds, equipment interlocks, and conflicts. Only after automatic approval or human confirmation according to risk does the command scheduler locate the relevant protocol drivers, translate the approved operations into Modbus register writes, and route them through the gateway. The platform combines response codes with setpoint readback to determine whether the two units actually executed.
...
The planning module first produces candidate actions: (1) pause that AGV's task; (2) reassign it to another AGV with sufficient charge; (3) return the low-battery AGV to a charging station; and (4) reroute both vehicles around the congested zone. Scheduling policy must check job priority, load state, right of way, obstacle avoidance, and safety zones. Only preapproved low-risk policy may proceed automatically; replanning outside those bounds requires human confirmation. The platform sends approved actions to the relevant driver services. After several iterations, the reasoning module reads point values and task receipts again to confirm that the first AGV is moving toward the charging station and the replacement has taken over on the planned route.

When the task class, action range, and safety constraints have all been approved in advance, this flow can run without step-by-step human confirmation inside those bounds; that is not unconditional AGV autonomy. The understand — plan — execute — understand loop closes through fresh evidence, policy validation, execution receipts, and state readback, not iteration frequency alone. Any boundary violation, conflict, missing receipt, or state mismatch must stop the automatic path and transfer control to a person.
