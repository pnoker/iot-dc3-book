...
First, time semantics. Modbus RTU is a half-duplex bus whose polling delay depends on baud rate, frame length, slave response, silent interval, retries, and device count; it cannot be summarized as “millisecond-level.” If a sensor provides a sampling time under a known synchronization contract, preserve it, and let the gateway add `Date.now()` as receive or poll time. Gateway time cannot stand in for the instant at which vibration was sampled. Their offset and clock quality belong in data-quality fields.
...
Third, uplink/downlink isolation must appear in topics, quotas, consumers, and permissions. `alarm-command` can be separated from uplink data, but its partition count and retention cannot be preset as “one or two is enough.” Derive them from command keys, ordering, consumer parallelism, recovery, and measured throughput. A threshold-change message also carries a version, authorized principal, expiry, and signature or trusted provenance; the edge validates it, applies it through a controlled update, and returns the result. Mosquitto receipt must not directly rewrite a Node-RED rule.
...
- **Peak-throughput estimation**: do not look only at average reporting frequency. Build a model from device count, points per report, normal and worst-case rate, synchronized triggers, backfill, and retries; a 1.5–2× burst multiplier is only a case assumption to validate. Carry “message TPS → point-write rate → network and replica amplification → disk → consumers and partitions” into the resource budget, then test it with real key distributions and failures.
- **Persistence and replication**: acknowledgment and replication semantics differ across messaging systems; there is no universal rule that every queue uses `acks=all` and at least two replicas. In Kafka, design `acks`, replication factor, `min.insync.replicas`, unclean leader election, and failure-domain placement together, then validate the acceptable loss window and recovery objective.
- **Failed-message disposition**: for malformed or persistently failing messages that may be skipped, design an isolation queue, retry limit, and manual disposition; also identify ordering-sensitive messages that cannot simply be bypassed. A DLQ can isolate some poison messages but does not repair the consumer, schema, or downstream service automatically.
...
| Test burst ingress | 3 messages/s × illustrative 2× burst factor | Start testing at 6 messages/s and 24 points/s, then revise from measured backfill and reconnect behavior |
| Application-payload lower bound | 6 messages/s × illustrative 200 B/message | About 1.2 KB/s; excludes protocol headers, TLS, retries, broker replication, and observability traffic |
| Kafka partition count | Determined by keys, ordering, consumer parallelism, and recovery | Cannot infer three partitions from 6 messages/s; compare skew, rebalance, and recovery across candidate counts |
| Time-series point ingress | Test target of 24 points/s | Batch size follows latency and request bytes; only a target-engine benchmark can identify the bottleneck |
| Logical points per day | 12 points/s × 86,400 s | About 1.04 million points; disk also includes measured bytes/point, indexes, WAL, compaction space, replicas, and backups |

The table exists to expose units, omissions, and test entry points, not to declare that a small system has “almost zero” capacity risk. Even at a low average load, broken retry logic, synchronized reconnects, a hot key, disk pauses, or downstream failure can create a peak. Write conclusions as benchmark pass/fail criteria and recompute them whenever devices, firmware, topology, or retention changes.

#### Time-Series Database Retention Policies and Query Patterns

- **Write throughput and batching**: writes and queries compete for CPU, memory, WAL, disk, and network; do not presume which ceiling is higher. Batching commonly amortizes request overhead, but choose batch bytes from latency, server limits, retry cost, and tests rather than a universal “hundreds to thousands of points” minimum.
...
- **Deriving index design from query patterns**: list frequent and expensive queries before deployment, then design against the target database's index, partition, and cardinality model. Whether device ID belongs in a tag, column, primary key, or partition key depends on the product. Validate with `EXPLAIN`, scanned bytes, and target-scale data rather than applying a Tag/Field slogan across products.
...
- **Remote operations channel**: once an edge node is deployed, it needs controlled diagnostics, but SSH or an arbitrary reverse proxy should not be exposed by default. Prefer device identity, least privilege, short-lived credentials, mutual authentication, audited operations, and revocable sessions; where a shell is necessary, restrict its source and commands. OTA packages need signature verification, staged rollout, atomic switching, and a rehearsed rollback appropriate to the hardware and boot chain.
...
- **Edge-model release**: full packages, differential packages, and weight-only updates each have trade-offs; a differential is not inherently more reliable. Whichever format is used, verify signature, hash, dependencies, disk space, and target-hardware compatibility, test a small cohort, switch atomically, and retain a bootable known-good version.
- **Rollback criteria**: false positives and false negatives are knowable only with labels or proxy measures, so “N alarms in a row” or “no alarms” cannot mechanically trigger rollback. Predefine stop conditions for data drift, runtime errors, latency, resources, human rejection, and safety events. Automatic or manual rollback passes through deterministic release control, while the edge may use only a preapproved fallback policy when disconnected.
...
- **IoT DC3**  
  The open-source industrial IoT platform cited throughout this chapter. The current code organizes Drivers alongside centers such as Gateway, Auth, Manager, Data, and Agentic, joining point values, commands, and receipts through gRPC business registration and messaging ports. It is useful for studying device access, metadata, and message contracts. Rule orchestration, model inference, time-series backends, and deployment topology must each be checked at the selected commit rather than inferred from the project name or an old diagram.
