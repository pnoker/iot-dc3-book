...
When network quality cannot guarantee "perfect delivery every time", the transmission path cannot avoid three engineering questions: What if a message is lost? What if a message is duplicated? How does a broken connection resume? The MQTT protocol provides the basic framework through three levels of Quality of Service (QoS). The packet-level mechanisms of QoS and their version differences are developed in Section 9.2 of Chapter 9; this section discusses only the reliability-engineering trade-offs, so the three levels are compressed into one sentence each: QoS 0 is at most once — the job is done the moment the message is sent out, and a lost message is not resent; QoS 1 is at least once — the sender retransmits when no acknowledgment arrives, so duplicates are possible; QoS 2 is exactly once — a four-step handshake (Publish → PUBREC → PUBREL → PUBCOMP) guarantees neither loss nor duplication, at the price of multiplied interaction overhead. The higher the level, the more reliable — and the slower and more expensive.
