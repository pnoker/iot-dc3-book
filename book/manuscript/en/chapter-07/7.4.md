...
Not every request needs to stay private. The approach the engineer eventually adopted is hybrid routing: simple device-status queries go to a local DeepSeek, fast and with data never leaving the site; complex fault root-cause analysis goes to the cloud GPT-5 to use its stronger reasoning; sensitive queries involving process parameters come back to the local model. The Agentic Center's `dc3_model_provider` table supports configuring multiple providers and selecting a model per session. The only missing piece for hybrid routing is the routing logic itself:
