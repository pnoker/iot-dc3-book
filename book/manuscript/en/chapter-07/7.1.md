...
**The first: compute moving to the edge.** Cloud inference introduces network latency, bandwidth cost, and data-residency risk, but "train in the cloud, infer at the edge, respond on the device" is a common split—not a fixed answer. Whether training data may be centralized, whether the model fits the target hardware, and whether a device-side response belongs to safety control must be assessed separately. Some edge devices can run compressed language or vision models within limited power budgets. Moving computation closer to the field can reduce raw-data egress and shorten part of the path, but it does not automatically satisfy privacy, real-time, or availability requirements; end-to-end benchmarks and failure drills must establish the benefit.

**The second: the leap in model capability.** Large language models have expanded from text generation to structured tool calls. Given a natural-language input such as "set the feed valve on Line 2 to a lower opening," a model can produce candidate APIs and parameters; the application and deterministic systems still validate parameter bounds, operating interlocks, authorization, and final execution. IoT DC3 uses Spring AI to map structured tool requests into governed Java method calls, while its device abstraction isolates protocol differences such as Modbus, OPC UA, and MQTT. This mapping reduces integration friction; it neither proves that the intent was understood correctly nor forms a safety boundary by itself.

Together, these forces make AIoT open to engineering validation; they do not establish autonomy. The following subsections examine the role of large models in the IoT (Section 7.1.2), how an Agent Runtime brings model capability into governed execution (Section 7.1.3), how RAG, Tool-Calling, and MCP connect models to controlled knowledge and tools (Sections 7.1.4 and 7.1.5), and how to evaluate a RAG system in layers (Section 7.1.6).
...
Once large models are plugged into IoT operations, they quickly encounter two concrete boundaries. First, model parameters do not automatically contain the variable-frequency drive commissioned last night, the register map updated this morning, or the operating procedure revised this quarter. Second, a model can produce text or a structured request, but it cannot place a command on the field bus by itself. Retrieval-augmented generation (RAG) lets the application supply traceable material at answer time; Tool-Calling lets the model produce a structured call candidate for the application to review, authorize, and execute. The two mechanisms expand context and interfaces; they do not remove questions of evidence quality, authorization, or physical execution confirmation.
