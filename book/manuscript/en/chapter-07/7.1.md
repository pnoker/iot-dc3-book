...
The official MCP specification (latest version 2026-07-28) defines only two transports: stdio and Streamable HTTP. stdio targets local scenarios — the host application launches the MCP server as a child process and exchanges JSON-RPC messages over standard input and output, typically for integrating local toolchains. Streamable HTTP targets remote scenarios — the client sends requests to one HTTP endpoint on the server, and the server may return incremental results as a streaming response on demand, fitting platform-scale, multi-tenant deployments. The HTTP+SSE transport from the early specification was deprecated as of the 2025-03 version, superseded by Streamable HTTP; WebSocket was never listed as an official transport. Some tutorials online still teach MCP through those two approaches, so check which specification version they rest on. IoT DC3's `POST /mcp` is exactly the Streamable HTTP form — which means the MCP endpoint is, first of all, a network-reachable HTTP service that must implement authentication, authorization, and access control to Web API standards, and must not be treated with the trust level of a "local process."

The protocol also defines a sampling capability (since the 2024-11 specification): while handling a request, the server may ask the client-side LLM, in reverse, to generate one completion — the server borrowing the client's model, in effect. DC3 does not enable this capability: model inference is deliberately concentrated under unified governance on the platform side, and opening reverse calls would send tenant data outside the controlled boundary and enlarge the audit surface.
...
**From MCP to A2A: interoperability among agents (outlook).** MCP answers "how does an agent call tools," while A2A (Agent-to-Agent, jointly proposed by several vendors in 2025, as of mid-2026) answers "how do agents discover one another, allocate tasks, and hand over execution" — the industry often sums up the two as "MCP gives agents a pair of hands; A2A gives agents colleagues." A2A describes capabilities through Agent Cards and supports task delegation, and is expected to scale gradually from 2027 in cross-system, cross-organization multi-agent scenarios (a vendor roadmap projection, not independently verified). The MCP specification itself is also evolving quickly: the 2026-07-28 version introduces the Tasks extension, providing create, query, and cancel state semantics for long-running server-side tasks; the official Registry (registry.modelcontextprotocol.io) has also gone live, serving as the authoritative distribution channel for the server directory. The right way to track these evolutions is against the specification text itself, not secondhand tutorials. The significance for the IoT is this: once a platform exposes device capabilities to external agents via MCP, agents of different kinds (equipment inspection, energy scheduling, production scheduling) still need A2A to coordinate the division of labor — precisely the seed of an "Internet of Agents." A complete AI-native platform should plan both layers of interoperability, MCP (the tool layer) and A2A (the orchestration layer); IoT DC3 currently implements the MCP tool layer, and A2A orchestration is a direction of platform evolution left to Chapter 14, "IoT DC3 in Practice," to elaborate.
