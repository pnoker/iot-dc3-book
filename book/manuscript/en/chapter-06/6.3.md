...
- `dc3-center-auth`: authentication, authorization, tenant, and OAuth/MCP management.
...
After a successful protocol read, `DriverSenderService.pointValueSender` publishes the standardized point value to RabbitMQ. The point value is not first written into a "device shadow + Redis" two-level cache maintained uniformly by the Driver. The current data path is:
...
`PointCommandReceiver` checks `expireAt` before execution, deduplicates by `commandId`, and uses a device-level lock to keep protocol operations for the same device from interleaving. The driver-specific command queues are also configured with TTL and a dead-letter exchange. Idempotency here rests on the command DTO and a local deduplication cache — it must not be fabricated into "every message deduplicated uniformly through a Redis Set or a database unique key."

#### RabbitMQ Is Currently the Only Message Middleware

The current code dependencies, environment variables, and Compose templates all revolve around RabbitMQ and include no Kafka broker, Kafka client, or Spring Cloud Stream Kafka Binder. Kafka remains a general option for other log-replay or very-high-throughput data pipelines, but it is not part of IoT DC3's current implementation and should not be written up as an established expansion route.
...
Table 6-3 lists the practice items that must be covered, across four dimensions. Each item has a corresponding actionable verification method; none relies on intuition.

**Table 6-3 IoT Microservice Engineering Checklist**
