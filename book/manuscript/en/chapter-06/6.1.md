...
Below is Python code for a temperature-and-humidity sensor simulating data transmission (example):
...
*Table 6-1 For comparison — actual differences depend on the specific implementation, degree of optimization, and business model.*
...
When the Paho client receives point values such as temperature and humidity reported by devices, what happens in the `messageArrived` callback is far more complex than the example — it must unpack the raw payload into semantically meaningful point structures and handle timestamps, thread pools, backpressure, and connection health. In IoT DC3, the Driver SDK currently publishes standardized point values to RabbitMQ, which the Data service then consumes; the project has no Kafka client or broker.
...
**Engineering value**: MQTT holds an irreplaceable niche at the edge. For battery-powered devices, high-latency networks, and intermittent disconnections, MQTT is the only sensible long-term choice. QoS levels should be chosen by business tolerance: QoS 0 for high-rate, losable data (such as temperature sampling); QoS 1 for commands that must arrive but tolerate duplication (such as remote configuration). QoS 2's two-way acknowledgment mechanism sees little actual use in IoT — its complexity exceeds the needs of most scenarios.

**Boundary**: MQTT is not a general-purpose data transfer protocol. Its broker is a single point; large-scale deployment needs a clustering scheme (such as EMQX or NATS). MQTT does not fit synchronous control scenarios with extreme real-time requirements — the asynchronous publish/subscribe model cannot guarantee millisecond-level response.
...
- **Device data reporting: MQTT first**. For battery-powered devices, unstable networks, and devices that can send only small amounts of data, MQTT is the only sensible choice. QoS 1 guarantees at-least-once delivery, and the broker can cache offline messages. Do not force REST or gRPC onto the device side — their resource consumption will drastically shorten battery life.
