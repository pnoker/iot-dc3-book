...
**Engineering value**: MQTT holds an irreplaceable niche at the edge. For battery-powered devices, high-latency networks, and intermittent disconnections, MQTT is usually the soundest long-term default. QoS levels can simply be chosen by business tolerance: QoS 0 for high-rate, losable sampling (such as temperature data); QoS 1 for commands that must arrive but tolerate duplication (such as remote configuration); the multi-round confirmation overhead of QoS 2 has little use in IoT scenarios and is rarely seen in practice.
