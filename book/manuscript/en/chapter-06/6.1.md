...
**Engineering value**: MQTT holds an irreplaceable niche at the edge. For battery-powered devices, high-latency networks, and intermittent disconnections, MQTT is usually the soundest long-term default. QoS levels should be chosen by business tolerance: QoS 0 for high-rate, losable data (such as temperature sampling); QoS 1 for commands that must arrive but tolerate duplication (such as remote configuration). QoS 2's four-way handshake (PUBLISH → PUBREC → PUBREL → PUBCOMP) mechanism sees little actual use in IoT — its complexity exceeds the needs of most scenarios.
