...
Choose an access protocol against the full deployment constraints. MQTT supplies broker distribution, QoS, and session semantics; whether messages are retained and delivered after an offline period depends on persistent sessions, subscriptions, message expiry, and broker storage. CoAP often fits resource-oriented interaction on constrained terminals and selects CON or NON per message, but UDP does not automatically mean lower latency or energy, and a CON ACK does not prove business-action completion. HTTP/HTTPS may serve devices, gateways, or northbound APIs. Its safety depends on authentication, TLS, authorization, exposure, and operations rather than on whether the endpoint is a device. Chapter 9 provides the fuller decision framework; the final choice still needs target-network, load, and failure testing.
...
Edge deployment does not replace the central cloud; it reallocates responsibility according to latency, bandwidth, and autonomy needs. Protocol drivers may run on edge gateways and exchange messages asynchronously with the data center. A bounded queue can absorb some WAN jitter, but capacity, overflow policy, and recovery determine the outage window it can tolerate. Edge filtering, aggregation, and local alarms can reduce cloud bandwidth and storage. A field workflow may be claimed to survive disconnection only when its required data, rules, credentials, time synchronization, control dependencies, and failure degradation close locally and have passed outage testing.
