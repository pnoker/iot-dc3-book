...
- **Device alarm stage**: temperature over-limit alarms, device-offline alarms, alarm severity tiers and delivery channels (SMS/app/dashboard pop-up).
...
What the requirements-analysis phase produces is not a "complete" requirements document — completeness is a myth: in IoT scenarios, protocol evolution, hardware iteration, and business change keep refreshing the requirements. The effective output is **an actionable requirements baseline and an explicit "what we will not do" list**. The latter is often more valuable than the former. How to map it into the system architecture design is the subject of the sections that follow.
...
Table 14-4 summarizes the core trade-off dimensions of the three deployment options. In real projects most solutions are combinations of the three — core services on the public cloud, key protocol drivers pushed down to the edge, and the private cloud carrying sensitive-data storage.
...
The second is **alarm-noise control**. IoT systems naturally produce large numbers of transient anomalies — network jitter, brief device offline spells, sensor-value jumps. Raise an alarm on every one and ops engineers will burn out within the hour. One practical strategy is duplicate-event silencing: once alarms of the same type appear at high frequency within a short window, the system automatically enters suppression — logging only, no repeated notifications.

Logs are structured (JSON or protobuf), collected by Filebeat or Fluentd, and shipped to a central Elasticsearch cluster. Once log volume reaches a certain scale, the ES cluster's storage and compute costs balloon quickly. A two-track scheme of sampling and retention is advisable: full logs kept for a few days, critical-level logs (WARN/ERROR) for several weeks, and statistics (message volume, error-rate trends) kept in the data warehouse for long-cycle analysis. The specific retention values should be assessed against business needs and hardware cost; there is no universal best value.

#### OTA Upgrades: Success or Failure Comes Down to One-Click Rollback

Firmware updates carry the highest operational risk. One bad firmware release can cut off an entire device fleet — where the field offers no physical recovery path, that is an operational incident.
...
2. **Upgrade transaction**: the upgrade flow should be transactional, like a database commit — download → verify signature → write to storage → switch to the new firmware → report the result. On failure at any step, the device must actively switch back to the previous stable version rather than hang in a half-updated state.
...
- **Restrict production change windows**: every change (configuration modification, driver upgrade, parameter adjustment) must pass an approval process, with complete, auditable change records. In an IoT system, one ops engineer casually changing a collection frequency can double the device traffic of an entire workshop and trigger a message-queue avalanche — that is not a hypothetical.

Further reading: Chapter 5 discusses the platform layer's resource management and elastic-scaling strategy, and Chapter 8 covers how device identity authentication and transport security are put into practice at deployment time.
