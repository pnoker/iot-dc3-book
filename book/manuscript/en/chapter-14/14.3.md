...
Engineering solutions often combine strategies: hot data uses storage selected for query latency, cold data moves to lower-cost object storage, deterministic high-frequency rules may run at the edge, and model inference is placed according to data, latency, and compute constraints. MQTT QoS must not be assigned mechanically as “commands save bandwidth, state gets reliability.” Discardable telemetry may use QoS 0, while messages requiring broker acknowledgment may use QoS 1 or 2. Every side-effecting device command also needs authorization, expiry, an idempotency key, a device receipt, state readback, and failure compensation. Loss, duplication, latency, session persistence, and business risk jointly determine the QoS.
