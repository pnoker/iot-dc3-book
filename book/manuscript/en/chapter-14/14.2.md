...
#### From Zero to the First Point: A 10-Step Smoke Sequence

Services being up does not count as a successful deployment — only the full chain running through does. The 10 steps below verify the three segments "infrastructure health — data uplink — command downlink" within a few minutes; the commands are based on the repository README and the official Quick Start as of August 2026. Image tags, ports, and paths evolve with versions — defer to the repository README/Compose, and treat details marked "illustrative" as teaching illustrations, not something to copy verbatim.
...
Expected: the list of Drivers started with the stack — a Driver appearing here means the gRPC business registration described in Section 14.2.3 has succeeded. Then follow the official Quick Start to create a profile, points (for example Temperature, FLOAT, READ_WRITE), and a device bound to that Driver, and record the deviceId and pointId. For the MQTT scenario, confirm `dc3-driver-mqtt` is in the list and the Compose file enables that service; if no external MQTT broker is available yet, the built-in virtual Driver can walk through the entire sequence.

**Step 6: Report one point value as the device.**

```bash
mosquitto_pub -h <broker-host> -p 1883 \
  -t '<device reporting topic as defined by dc3-driver-mqtt>' \
  -m '{"deviceCode":"<device>","values":{"Temperature":25.6}}'   # topic and field structure are illustrative; defer to the driver implementation
```

Expected: if the command returns normally, the publish succeeded. Note the factual boundary from Section 14.2.4: the MQTT topic and payload structure are defined by the specific Driver — the above is only an illustration of the structure. The MQTT Driver needs a reachable broker, with address and port set in that driver's configuration.
...
Expected: the latest records for that point are returned (fields such as rawValue, numValue, and createTime) — proof that the uplink path "Driver → RabbitMQ → Data → PostgreSQL" is through.
...
podman logs dc3-driver-mqtt --tail 50     # or whichever driver container is actually enabled
```

Expected: the Data log shows point-value consumption and save records, and the Driver log shows registration and read/write execution records; then cross-check the RabbitMQ management console to confirm the relevant queues have no backlog and no dead letters. At this point, both the uplink and downlink paths have inspectable evidence.
...
| Data reporting | Publish one message as the device with `mosquitto_pub` (step 6 of the 10-step sequence above); for Modbus, keep the registers readable with a slave simulator such as diagslave | Point values enter Data via RabbitMQ and are written to PostgreSQL |
