...
**Device access module**: responsible for protocol adaptation. The smart factory involves Modbus TCP, serial links (custom protocol), and some newer devices that support MQTT (Message Queuing Telemetry Transport). Different protocols map to different drivers; the drivers run close to the field devices, and the collected data is reported to the cloud through a message queue rather than connecting directly to the core services. This layer does no data storage — only format conversion and data forwarding.
...
### 14.2.5 Implementing AI Operations Features
