...
GDPR has several direct effects on engineering architecture. The **data-minimization principle** requires personal data to be adequate, relevant, and limited to what is necessary for a stated purpose. If a smart-bulb vendor also collects Wi-Fi signal strength and ambient noise, it must document the purpose, legal basis, and retention period. **Transparency and legal basis** require the platform to record why each class of processing occurs. Consent is only one possible legal basis; when processing does rely on consent, withdrawal should be as easy as giving it. The **data-breach notification duty** requires a controller to assess an incident and notify the supervisory authority when the statutory conditions apply. Engineering therefore needs a queryable processing inventory, incident timeline, affected scope, and response evidence; the presence of logs alone is not compliance.

GDPR Article 17 establishes the **Right to Erasure**, also called the Right to Be Forgotten, but it is not an unconditional command to purge every copy. A request must meet an applicable ground, and exceptions may cover freedom of expression and information, legal obligations, public interest, research or archiving, and legal claims. Backups cannot be reduced to "erase every fragment immediately" either. The system should maintain a data inventory, recipients, and retention periods; carry erasure or restriction through production copies, caches, edge nodes, and warehouses; and prevent erased records from silently returning to production after a backup restore. The precise procedure must reconcile regulatory guidance, recovery integrity, and legal-retention duties, with legal and security review.
...
| Data security | Personal-data scope, legal basis, and retention are defined; erasure/restriction workflows and backup-restore controls are implemented | GDPR, MLPS 2.0 |
...
This checklist is not a complete diagnostic tool, but it exposes questions engineering teams must answer during design: What personal data does the system process, on what legal basis, and for how long? How does an erasure request propagate, and how does a restore prevent deleted data from reappearing? Is sensitive data encrypted in transit and at rest? Who may access which data—and is permission allow-by-default or deny-by-default? Answering these questions only after launch costs far more than writing them into the architecture and operations manuals during design.
