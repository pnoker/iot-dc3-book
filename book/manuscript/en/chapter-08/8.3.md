...
Some IoT devices choose UDP to avoid maintaining connection-oriented transport state and to select acknowledgment and retransmission per message. Whether that lowers latency or energy still depends on radio access, handshake frequency, and application behavior. CoAP (Constrained Application Protocol) has a four-byte minimum fixed header and fits constrained networks. TLS is designed for a reliable, ordered byte stream and cannot simply be placed over datagrams that may be lost, reordered, or duplicated. The issue is not one sequence-number field but the record layer's and handshake's overall transport assumptions.

DTLS (Datagram Transport Layer Security) adapts TLS security goals to datagram transport and handles reordering, loss, and replay windows. RFC 6347 defines DTLS 1.2 and [RFC 9147](https://www.rfc-editor.org/rfc/rfc9147) defines DTLS 1.3; version choice depends on endpoint and server support. CoAP can use DTLS as CoAPS, while deployments may also evaluate object security such as OSCORE. LwM2M security modes and transport bindings likewise depend on the adopted specification revision and must not be summarized as “always DTLS 1.2.”
...
The engineering dividing line is clear: high-value, low-frequency, high-consequence control commands such as remote firmware updates and emergency shutdowns should generally use digital signatures to support origin authentication, integrity checking, and accountability. Whether this amounts to legal or organizational "non-repudiation" also depends on private-key control, identity binding, trusted time, and the audit regime. High-frequency telemetry can use HMAC to reduce computational cost. The two mechanisms are not mutually exclusive and can be combined.
...
The three common schemes emphasize different trade-offs. Timestamps are simple, but the receiver must validate an acceptance window and the clocks must be synchronized closely enough; a wide window expands replay opportunity, while a narrow one rejects legitimate traffic. Monotonically increasing sequence numbers need no clock and can identify individual messages, but they require persistent receiver state; reboot recovery and sequence jumps are engineering problems. Random nonces need no global clock, but the receiver must retain a seen set for their validity period; only a receiver-issued challenge adds an extra round trip. No single scheme is "most thorough" independently of state capacity, packet loss, reboot behavior, and the threat model.

In practice, the three schemes are often combined. MQTT 5.0's Session Expiry Interval only specifies how long session state remains after disconnection; Packet Identifiers support QoS flows and recovery of unacknowledged packets. Neither is a security anti-replay mechanism across sessions. If application messages must resist malicious replay, use authenticated business sequence numbers, timestamps, or nonces in addition to TLS, and define persistence, acceptance windows, and reboot behavior. For CoAP, the IETF-standardized Object Security for Constrained RESTful Environments (OSCORE) protects CoAP messages at the application layer and uses integrity-protected sequence numbers with a replay window for message-level replay protection; the mechanism is described below.
