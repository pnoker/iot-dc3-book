...
Earlier chapters showed AI moving from passive answers toward candidate judgments and tool calls. A model should not directly control locks, valves, or motors. Even so, a compromised model can propose high-risk actions and try to induce the governance path to approve them, with consequences far beyond an ordinary text error. Malicious training examples or tampered model artifacts can implant backdoors, so risk may lie not only in application code but also in trusted weights, data, and the model supply chain.
...
Whether the attack is prompt injection or jailbreaking, output governance is an important defense, but not a “final line” by itself. Treat model output as an untrusted candidate Tool call, then apply independent authorization, schema and range validation, policy, confirmation, and the deterministic control path. Only if those boundaries are misconfigured or bypassed can a bad “write point” candidate become a physical consequence such as opening a valve or lock or starting a motor.
