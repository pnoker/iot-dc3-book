...
One more word on the authorization framework. MCP's authorization specification is based on OAuth 2.1 (an IETF draft RFC that consolidates best practices on top of OAuth 2.0, such as mandatory PKCE and removal of the implicit flow); its resource indicator (RFC 8707) confines a token's audience to a specific resource server, preventing a token issued for tool A from being carried off to call tool B — exactly the token-layer solution to the Confused Deputy problem described in Section 8.5.4; the access side adopts the idea of dynamic client registration, letting a client register itself at first connection instead of provisioning long-lived credentials in firmware. The checkpoints for this mechanism appear in Section 7.6 of Chapter 7 as CHK-10, and Section 9.5 returns to this when it discusses the MCP protocol.
