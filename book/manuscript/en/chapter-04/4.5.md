...
The project's immediate challenge comes from protocol differences. NB-IoT streetlights and LoRa streetlights differ almost completely in communication link, data-reporting mechanism, and packet structure. Table 4-4 summarizes the key protocol comparison between the two device types.

**Table 4-4 Smart streetlights: protocol and communication comparison of the two device types**
...
A driver is the execution unit of protocol adaptation — an independent microservice that encapsulates a specific protocol's connection, data-parsing, and command-dispatch logic. The streetlight project needs the NB-IoT driver and the LoRa driver deployed.

**Upload and startup flow**:
1. **Obtain the driver packages**: Write or obtain the NB-IoT and LoRa driver packages (or container images) against the IoT DC3 Driver SDK. The driver implements the required fine-grained SPIs; at startup it completes driver and attribute business-metadata registration without depending on a service registry.
2. **Upload to the platform**: In the admin console's "Driver Management" module, fill in the driver name (e.g., `dc3-driver-nbiot`), the version number, and type tags.
3. **Start the instance**: After you click "Start", the platform deploys it as an independent microservice instance. Check the log module for the output "Driver nbiot-server started, registered to center". Once the status changes to "Online", the driver is ready.
