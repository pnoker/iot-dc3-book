...
The **Driver SDK** takes the abstraction down to the code level. An IoT platform that needs to integrate dozens of protocols cannot pile all parsing logic into the platform's main process — coupling would be extreme, and upgrading any one protocol could affect other modules. The more workable approach is to agree on the driver's conceptual interface — read by point (`read`), write by point (`write`), and the link heartbeat, plus connection lifecycle management (for the framing, see Section 4.3.3) — and package each protocol as an independent driver service that communicates with the platform through message channels. IoT DC3 ships with **36 protocol drivers**, each an independent microservice that registers with the management center at startup, collects data by point, and executes write operations on command.
