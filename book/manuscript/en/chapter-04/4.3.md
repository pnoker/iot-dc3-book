...
- **Property**: the device's state variables, readable or writable — for example, a temperature sensor's current temperature or a smart lock's locked state. Property values are instantaneous sampled data.
...
Whether the data arrives as NB-IoT CoAP messages or LoRaWAN FPort payloads, once the data has been parsed and filled into property, event, or service instances, the upper layer sees the uniform `{"temperature": 25.3}` — no longer `0xA8 0x13` or `0x0F 0x00`.
...
Below is a JSON Schema example of a thing model for a temperature and humidity sensor, defining two properties (temperature, humidity) and one event (temperature over-limit alarm).

```json
{
  "$schema": "http://json-schema.org/draft-07/schema#",
  "title": "Thing Model for a Temperature and Humidity Sensor",
  "description": "A generic model for indoor environment monitoring, independent of the underlying protocol",
  "type": "object",
  "properties": {
    "temperature": {
      "type": "number",
    ...
    "humidity": {
      "type": "number",
      "description": "Current ambient humidity percentage",
      "minimum": 0,
      "maximum": 100,
      "unit": "%RH",
      "access": "R"
  ...
  "events": {
    "temperature_alarm": {
      "description": "Temperature exceeds the configured threshold",
      "outputData": {
        "type": "object",
        "properties": {
          "threshold": { "type": "number", "unit": "°C" },
          "actual": { "type": "number", "unit": "°C" }
        }
      }
    }
  },
  "services": {
    "set_sampling_interval": {
      "description": "Set the data sampling interval",
      "inputData": {
        "type": "object",
        "properties": {
...
An adapter does not choose when or by whom it gets used. The framework needs a "driver catalog" so that when a new device comes online, the framework automatically finds a suitable adapter. The common industry approach is **service registry + label matching** (note: this is the generic architecture pattern, not how IoT DC3 works — DC3 has no service registry; its driver discovery relies on business-metadata registration, as described at the end of this section). On startup, each driver publishes its description to the registry — protocol name, supported point types, connection-parameter patterns, and so on. If a device's configuration carries a `protocol=mqtt` label, the framework goes to the registry and pulls every driver service tagged `mqtt`.
...
IoT DC3's built-in protocol drivers are organized as independent microservices — but there is no service registry and no unified base-class abstraction. Drivers complete their business-metadata registration at startup; protocol implementations plug into the SDK through fine-grained SPI interfaces; point commands, point values, and status events flow through the message queue. This is the grounded answer to the engineering challenge of "protocol fragmentation": whatever the underlying protocol — BLE, Modbus, or OPC UA — the central services always face a stable data model and stable message contracts.
