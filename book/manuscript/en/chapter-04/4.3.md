...
An adapter does not choose when or by whom it gets used. The framework needs a "driver catalog" so that when a new device comes online, the framework automatically finds a suitable adapter. The common approach is **registry + label matching**. On startup, each driver publishes its description to the registry — protocol name, supported point types, connection-parameter patterns, and so on. If a device's configuration carries a `protocol=mqtt` label, the framework goes to the registry and pulls every driver service tagged `mqtt`.
