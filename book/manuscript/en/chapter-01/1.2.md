...
In this example, the connected objects are workshop sensors and controllers, the transmitted data consists of point values flowing machine to machine (M2M), and the system's value appears in predictive maintenance and efficiency optimization. The loop in the figure is not a model connected directly to equipment: the model produces a risk assessment or candidate action, while authorization, policy, confirmation, audit, and the deterministic control chain still constrain every write. That is what distinguishes the Internet-of-Everything era from the previous two waves: connection is the means; verifiable sensing, reasoning, and action in the physical world are the goal. The move from data collection to system intelligence is reshaping the information architecture and providing a foundation for governed deployment of AI models in IoT scenarios.

**In sum**: each wave redefined “who gets connected” and “what the connection is for” — the PC connected information, mobility connected people, and the Internet of Everything connects things and systems. In the first two waves, endpoints were people and value was driven by information consumption; the third expands endpoints to things, with more value coming from **data-driven, governed coordination among devices**. This difference echoes the limits of industrial software discussed in Section 1.1 — strong in deterministic control, weaker in adaptive intelligence. The next step is not simply to replace SCADA or MES, but to preserve deterministic control boundaries while adding verifiable sensing, reasoning, and action to the connected system. Next we turn to the standard definition and core elements of the IoT.
