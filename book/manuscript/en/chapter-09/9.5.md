...
MCP (Model Context Protocol) emerged against exactly this background. It is not a device protocol; it is a session protocol designed specifically for interaction between AI applications and external tools, resources, and knowledge bases.

Before going further, it helps to separate two kinds of statements. Where IoT DC3 is concerned, what follows cites the publicly documented design direction of the project's MCP implementation — a factual account of the project as it stands today. Where the application and value boundaries of this class of protocols in IoT are concerned, the reading is the author's engineering judgment, not a final definition of the formal standard, and some details are presented illustratively.
...
- **Tools**: executable actions that can be triggered by a model request. Each tool declares its own input schema, describing parameter names, types, constraints, and whether they are required. The AI model does not execute tools directly; it generates a call request, and the MCP server verifies permissions, performs the operation, and returns the result. IoT DC3 follows a "one gateway plus four center services" architecture — the Auth center handles authentication and authorization, the Manager center manages device and configuration metadata, the Data center carries the data pipeline, and the Agentic Center provides AI agent capabilities — and its MCP Server auto-generates the tool catalog by aggregating the OpenAPI interfaces of these four center services.
...
The IoT DC3 MCP gateway exchanges messages over JSON-RPC 2.0. Client and server complete initialization through capability negotiation, declaring the protocol versions and capability lists they support. The server returns the list of available tools through `tools/list`, and the client triggers tool execution through `tools/call`. The entire interaction is stateful — every call carries a request ID, responses are correlated with requests within the session, and timeouts and error handling are supported. Communication goes through token-based authentication (the OAuth 2.1 authorization mechanism is covered in Chapter 8, Section 8.5.3), combined with a tool whitelist and risk tiering, ensuring that every call takes place under the established security policy.

Figure 9-9 shows a typical interaction sequence of the MCP protocol in an IoT scenario, covering resource discovery, tool invocation, and state feedback.
...
MCP uses JSON-RPC 2.0 as its message carrier. This choice is not the technical optimum — if minimizing payload size were the goal, CBOR or protobuf would fit better — it exists to lower the entry barrier on the AI application side: any language that supports JSON serialization can process the messages directly, without pulling in an extra serialization library. As for transport, the official MCP specification (latest version dated 2026-07-28) defines only two: stdio and Streamable HTTP — the former launches the MCP server as a local subprocess and exchanges messages over standard input and output; the latter targets remote deployment, where the client sends requests to a single HTTP endpoint on the server and the server can stream back incremental results on demand. The HTTP+SSE transport from the early specification was deprecated as of the 2025-03 version, superseded by Streamable HTTP; WebSocket was never listed as an official transport, and IoT DC3's `POST /mcp` is of the Streamable HTTP form. The specification itself is still evolving quickly: the 2026-07-28 version introduced the Tasks extension, providing create, query, and cancel state semantics for long-running server-side tasks; the official Registry (registry.modelcontextprotocol.io) has also gone live as the authoritative distribution channel for the server directory. Chapter 7, Section 7.1.5 discussed these mechanisms from the standpoint of IoT DC3's implementation boundary; what this section covers is the protocol layers and the standardization status themselves. But the boundary must be clear: MCP does not define device-side envelopes, frame headers, or payload formats — those are the business of MQTT, CoAP, or HTTP. MCP solves exactly one problem: how an AI application discovers and invokes external capabilities, and how external capabilities describe themselves.
