...
The IoT DC3 MCP gateway exchanges messages over JSON-RPC 2.0. Client and server complete initialization through capability negotiation, declaring the protocol versions and capability lists they support. The server returns the list of available tools through `tools/list`, and the client triggers tool execution through `tools/call`. The entire interaction is stateful — every call carries a request ID, responses are correlated with requests within the session, and timeouts and error handling are supported. Communication goes through token-based authentication (this project adopts OAuth 2.1; see Chapter 8, Section 8.5), combined with a tool whitelist and risk tiering, ensuring that every call takes place under the established security policy.
