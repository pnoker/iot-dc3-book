...
| Firmware update | 5 | Perform firmware update | 1 | Execute | Triggers the upgrade procedure |
| Firmware update | 5 | Firmware state | 2 | Read | Upgrade progress/status code |
...
To understand the combined value of CoAP and LwM2M in NB-IoT, a curbside urban parking scenario is more intuitive than any abstract description. **Here is the example**: a certain city deployed over a thousand geomagnetic sensor nodes, each attached through an NB-IoT module, periodically reporting "free/occupied" status and supporting remote adjustment of billing-policy parameters (such as the free duration and peak-rate thresholds) as well as firmware upgrades. In this system, NB-IoT provides the wide-coverage, low-power physical channel, CoAP handles lightweight message exchange, and LwM2M carries device management and object standardization — the three working in concert are the key to low-power operations.

#### Fitting CoAP NON Messages to NB-IoT Power-Saving Mechanisms

NB-IoT is a radio access technology introduced in 3GPP Release 13, with a physical layer designed for deep coverage and low-power, small-packet transmission. The standard introduces two power-saving mechanisms: eDRX (Extended Discontinuous Reception) and PSM (Power Saving Mode). Devices remain asleep most of the time, waking only in configured paging windows or to report proactively. This fits naturally with CoAP's connectionless, stateless model.
