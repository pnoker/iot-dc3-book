...
- **CON messages**: after the sender issues a CON request, a receiver that can process the message normally returns an ACK (Acknowledgment); one that cannot may return RST. If no ACK arrives before timeout, the sender retransmits with exponential backoff until it receives a response or exceeds the maximum retransmission count. An ACK confirms a CoAP exchange, not completion of a physical action; closing the business loop still requires response codes, idempotency keys, device receipts, or state readback.
...
One candidate design is to send a CoAP NON (Non-confirmable) message after detecting a magnetic-field change, then enter PSM deep sleep. NON requires no ACK, has no protocol-layer retransmission, and maintains no long-lived connection session, so the device state machine can become “sample — packetize — send — sleep.” If a billing event needs a higher probability of message arrival, CON (Confirmable) can add exponential-backoff retransmission. CON can still fail after the retransmission limit, however, and it cannot prove that billing or gate operation completed. Critical workflows therefore also need event identifiers, deduplication, application receipts, timeout compensation, and reconciliation. Whether CoAP + NON + PSM consumes less energy than a TCP design must be measured on the actual operator network, coverage level, and reporting interval.
...
Operators send Write operations through the LwM2M Server; the client may use CoAP CON exchanges to improve message-arrival probability. The sensor validates and updates its configuration, returns a result, and the platform uses readback to confirm that the change took effect. Firmware upgrade is a representative LwM2M operation: the client may download a firmware binary in fragments through CoAP block transfer. Object 5 does not automatically guarantee resumability; that depends on whether the client, server, and transport implementation preserve block state and support recovery.
...
Using NON messages for geomagnetic reports is a classic power-versus-reliability trade-off. Consecutive loss may cause the platform to infer the wrong state and affect billing. A backend can compensate with the latest state, timeouts, and later samples, but it must label an inference rather than present it as a measurement. Critical workflows such as billing or gate opening may use CON to improve exchange reliability; confirmation extends the wake window, and an ACK is still not evidence that the business action succeeded. The engineering checkpoint is distinguishing **state redundancy, message confirmation, and action completion**, with business receipts and reconciliation for the latter two.

Block-transfer reliability for firmware upgrade is more complex because the device may lose power during download. LwM2M Object 5 defines resources for the firmware package, update action, state, and result, but resume behavior still depends on the client, server, and block-transfer implementation. Without persisted block or download state in nonvolatile storage, the device may restart from zero after power loss. Deployment tests should cover power interruption, weak networks, duplicate requests, and rollback rather than inferring resumability from Object 5 support alone.
