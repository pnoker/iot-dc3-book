...
RAGAS 研究将 RAG 质量拆为检索相关性、回答对检索内容的忠实程度以及最终回答质量等维度；其论文初版发表于 2023 年，并在 2025 年修订（[RAGAS: Automated Evaluation of Retrieval Augmented Generation](https://arxiv.org/abs/2309.15217)，访问于 2026-07）。工程评测至少应包含：
...
### 7.3.4 PointValueTool：实时数据、历史数据与受控写入
...
- **任务成功率**：最终状态与目标状态一致的任务所占比例。对于一个“查询某条产线过去一小时的温度曲线”任务，成功意味着返回了正确的位号值和时间戳，并且模型没有添油加醋。对于一个“将空调设定温度从 24℃ 调到 22℃”任务，成功意味着设备返回的回执中确实显示 setPoint 变为 22，且下一轮状态拉取确认。
...
- **人工接管率**：多少任务最终需要操作员介入修正结果或重新执行。假如一个 Agent 有 30% 的任务最终需要人工重做一遍，那它不仅没有提效，反而增加了现场工作量。
- **完成时限达标率**：对于有 SLI（服务等级指标）约束的场景——如“电梯困人后 2 分钟内远程手动复位”——系统必须在阈值内完成全链路，超时即视为失败，即便最终状态正确（人已经被救出，设备确实执行了指令，但已经晚了）。
...
- **端到端时延 PERCENTILE**：P50、P95。P95时延如果超过任务容忍度（如 30 秒），说明大量用户在正常使用中可能会遇到多次超时；P50 则反映常规情况下流畅度。
...
|---|---|---|---|
| 结果 | 任务成功率 | 最终平台/设备状态与目标一致 | ≥ 85% |
| | 正确拒绝率 | 模型主动拒绝越权请求 | ≥ 95% |
| | 人工接管率 | 模型结束后人工修正次数 | ≤ 10% |
| 轨迹 | Tool / 参数正确率 | 工具选择与参数准确率 | ≥ 90% |
| | 无效重复调用率 | 同一 Tool 无信息新增的重复调用 | ≤ 5% |
| | 重试行为正确率 | 可重试/不可重试/人工场景正确区分 | ≥ 80% |
| 安全 | 安全通过率 | 负向用例全部通过 | 100%（0越权/0审批绕过） |
| 代价 | P50 / P95 时延 | 端到端任务耗时 | 按任务容忍度设定：常见P95≤30s |
| | 成功任务成本 | 每正确完成任务的 token / 货币 | 比人工操作至少节省 40%（估算） |
...
>     - 跨租户越权率 = 0%
>     - 不可逆动作自动执行率 = 0%
>     - 其他阈值（如任务成功率 ≥ 85%，人工接管率 ≤ 10%）按场景风险定级，不做硬性压制。
