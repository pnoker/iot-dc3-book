...
id: "fig-14-07"
type: framework
title: 图14-7 需求分析的四个来源与资产生命周期检视
audience_takeaway: "读者应理解需求来自访谈、现场勘查、存量系统、行业规范四个来源，用资产生命周期检视法从部署到退役逐阶段推导用例，再以 MoSCoW 划分优先级并量化非功能需求。"
purpose: 展示需求获取的四个来源与资产生命周期检视法
visual_focus: 四个来源卡片与资产生命周期五阶段。
design_level: framework
layout: 上半四个来源卡片，中部生命周期五阶段流程，底部 MoSCoW 与非功能需求
elements:
- 四个来源：用户访谈（场景级需求）、现场勘查（物理约束）、存量系统（接口/字段映射）、行业规范（合规底线）
- 资产生命周期：部署 → 运行 → 告警 → 维护 → 退役，逐阶段推导用例
- MoSCoW：Must / Should / Could / Won't，把 Must 收窄到最少
- 非功能需求：可靠性、安全性、可扩展性、实时性，决定架构与成本
relationships:
- 四来源产出需求条目；生命周期法逐阶段推导用例；MoSCoW 划分优先级
caption: 图14-7 需求来自访谈、现场勘查、存量系统、行业规范四个来源，用资产生命周期检视法从部署到退役逐阶段推导用例，再以 MoSCoW 划分优先级并量化非功能需求。
render_notes: HTML/SVG 渲染，四来源卡片 + 生命周期五阶段流程 + 底部 MoSCoW 与非功能对比，蓝绿主色。
```
...
id: "fig-14-09"
type: matrix
title: 图14-9 部署形态选择与边缘节点管理
audience_takeaway: "读者应理解公有云、私有云、边缘三种部署形态对应不同权衡，边缘节点管理需解决状态感知、配置分发、版本管控，OTA 依赖差分、事务与灰度。"
purpose: 对比三种部署形态并展示边缘节点管理与 OTA 要点
visual_focus: 三列部署形态卡片与边缘管理三件事。
design_level: matrix
layout: 上半三列部署形态（公有云/私有云/边缘），下半边缘节点管理三件事 + OTA 三能力
elements:
- 公有云：运维低、账单易超预期、典型智慧城市/车联网
- 私有云：掌控强、需自扛高可用、典型工厂/园区/医疗
- 边缘部署：可离线自治、引入节点管理成本、典型工业现场/矿山/港口
- 边缘管理：状态感知（心跳 1.5 倍）、配置分发（中心化）、版本管控（可回滚）
- OTA：差分升级、升级事务、灰度发布
relationships:
- 三种形态对应不同数据主权与运维能力，实际多为组合；边缘管理三件事支撑节点治理
caption: 图14-9 公有云、私有云、边缘三种部署形态对应不同权衡；边缘节点管理需解决状态感知、配置分发、版本管控三件事，OTA 升级依赖差分、事务与灰度保证一键回滚。
render_notes: HTML/SVG 渲染，三列对比卡片 + 底部边缘管理与 OTA，蓝绿橙三色。
```
