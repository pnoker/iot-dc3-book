...
legend:
  ...
  - "虚线箭头=含误差的测距链接。"
caption: "图3-6 三角测量定位原理示意图。三圆在理想条件下应精确交于一点，但实际测量存在RSSI波动或ToA时钟误差，导致交叠区域出现，可借助最小二乘法计算误差平方和最小的点作为最终坐标。"
visual_constraints:
...
id: "fig-03-14"
type: dataflow
title: 图3-14 边缘数据预处理的三个工程目标
audience_takeaway: "读者应理解边缘预处理围绕滤除噪声、减少数据量、独立决策三目标，上传干净数据、向下快速动作，只在边缘执行无需跨设备上下文的操作。"
purpose: 展示边缘数据预处理的三个工程目标
visual_focus: 三目标自上而下的数据流。
design_level: dataflow
layout: 三个目标纵向堆叠（滤除噪声 → 减少数据量 → 独立决策），底部工程准则
elements:
- 滤除噪声：滑动平均、IIR 低通、中值滤波
- 减少数据量：聚合（均值/最大/最小/峰值）、FFT 频谱特征
- 独立决策：阈值规则触发本地动作 + 事件上下文上报
relationships:
- 三目标依次递进，上行流量压缩到原始量一个数量级以下，本地响应压缩到采样周期级
caption: 图3-14 边缘预处理依次滤除噪声、减少数据量、独立决策，向上传干净数据、向下做快速动作；只在边缘执行无需跨设备上下文的操作，复杂分析留给网关或云平台。
render_notes: HTML/SVG 渲染，三目标纵向堆叠，蓝绿橙分层，箭头表示数据流方向。
```
