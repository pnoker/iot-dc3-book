...
- **应用篇·场景实践与智能体应用**（第10-14章）回答"智能如何落地"——
  从工业制造到智慧城市、农业、区块链，并在第14章用 IoT DC3 项目实战收束，看技术如何在真实场景中创造价值。
...
若章节需要案例支撑，会以真实的工程问题开篇，并用 IoT DC3 等实际项目作为参照，
但全书主线仍然是通用架构、标准与工程方法。
...
- **应用篇 · 场景实践与智能体应用**（第10章《工业物联网与智能制造》, 第11章《智慧城市与车联网》, 第12章《农业物联网与环境监测》, 第13章《可信数据协作、分布式身份与隐私计算》, 第14章《IoT DC3 项目实战：从平台到智能体应用》）
...
Agentic 子层（Agentic Layer）不是一个独立于应用层之外的第五层，而是应用层内部的一个AI推理与执行编排子层。它运行在平台层提供的结构化数据之上，核心职责拆成三块：
...
设备管理是基础。Manager 中心真正的价值在于内嵌的规则引擎，其核心是事件-条件-动作模型，简称 ECA：
...
![图4-8 IoT DC3 驱动启动与业务注册时序](figures/chapter-04/fig-driver-load-sequence.png){width=15cm}

*图4-8 驱动启动后先经 gRPC 向 Manager 同步业务元数据，再初始化协议资源与调度任务；运行时通过 RabbitMQ 收发命令、回执和状态。业务注册不等于服务注册中心。*
...
*图14-2 IoT DC3 四层架构：北向请求经 Gateway 进入四中心，Driver 经 gRPC 对接 Manager，命令与数据经 RabbitMQ 在 Data 与 Driver 之间异步流转。*
...
*图14-3 IoT DC3 设备数据链路：协议 Driver 将位号值经 RabbitMQ 交给 Data，Data 更新最新值缓存并写入 PostgreSQL；点位命令沿反向异步链路执行并回传结果。*
...
*图14-4 预测性维护扩展示例：模型在平台外训练和部署，授权任务把推理结果作为衍生位号写回 Data，复用已有规则与通知链路。*
...
*图14-5 IoT DC3 当前容器化部署：PostgreSQL 与 RabbitMQ 提供基础设施，Gateway 和四中心组成平台，协议 Driver 按需启用并通过固定服务名与消息契约接入。*
