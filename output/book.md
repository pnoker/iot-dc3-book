...
![移动互联网应用生态示意](figures/chapter-01/fig-1-2.png){width=15cm}
...
![图1-3 物联网系统五要素关系示意](figures/chapter-01/fig-1-3.png){width=15cm}
...
![图1-4 四层物联网参考架构示意图](figures/chapter-01/fig1-4.png){width=15cm}

*图1-4 四层物联网参考架构。数据从感知层上行至应用层，指令以相反方向下行，形成闭环控制。安全条带从左侧贯穿，跨越所有层次。*
...
![图1-4 RFID 系统基本组成](figures/chapter-01/fig-01-04-2.png){width=15cm}

*图1-4 RFID 系统基本组成：标签、读写器与后台系统的物理/逻辑关系。*
...
![图1-5 物联网平台市场格局示意](figures/chapter-01/fig-1-5.png){width=15cm}

*图1-5 示意展示2020年前后物联网平台市场在生态完整性与运营商支持力度两个维度的差异化定位。气泡位置与大小仅为教学示意，不代表真实市场份额。*
...
![图1-5 物联网价值迁移路径](figures/chapter-01/fig-1-5-2.png){width=15cm}

*图1-5 物联网产业价值重心从连接层向平台层、最终向智能决策层迁移的路径。*
...
图1‑7对比了两种模式的决策链路。左侧是传统的规则驱动路径：用户通过固定控制面板或App输入指令，规则引擎精确匹配后直接驱动设备执行。右侧是引入大语言模型后的新路径：用户用自然语言描述需求，大模型解析意图、查询设备实时状态，生成决策方案并展示推理依据，最后交由用户二次确认后再执行。

![图1-7 传统物联网与大模型驱动物联网的对比](figures/chapter-01/fig-1-7.png){width=15cm}

*图1-7 传统物联网与大模型驱动物联网的对比示意。左侧为规则驱动的“感知-响应”模式，右侧为推理驱动的“理解-决策-确认”模式。大模型在决策层增加上下文理解和概率推理的能力，同时引入了执行前二次确认的安全机制。*
...
![传统物联网端-云架构与AIoT边-端-云协同架构对比](figures/chapter-01/fig1-6.png){width=15cm}
...
![AIoT数据闭环流程](figures/chapter-01/fig-1-5-1.png){width=15cm}
...
![图1-5-2 边云协同AIoT三级架构示意](figures/chapter-01/fig-1-5-2-1.png){width=15cm}
...
![实践建议优先级矩阵](figures/chapter-01/figure-1.16.png){width=15cm}
...
经典四层架构（Classic Four-Layer Architecture）预设了“数据最终给人看、人来做决策”的链路。部署百万级设备的团队在应用层卡住，根源就在于此：数据上来了，但缺乏统一语义，规则引擎在工况变化时频繁误报，想分析趋势却找不到完整的数据上下文。AI时代的到来，尤其大语言模型（LLM）和边缘智能的实用化，正在打破这个预设——机器不仅能理解数据，还能自己规划、自己执行。但经典四层架构中，应用层作为顶层，职责中并未为“理解、决策、执行”这套闭环划出一个标准化的专属位置。

#### 来自云端和边缘的两股驱动力

这条闭环为什么必须存在？因为它来自两个方向的技术压力。

第一个方向来自云端：大语言模型的实用化。一张位号值表记录着“37.5℃”，但一个在技术文档和运维日志上训练过的模型，能理解这个37.5℃对应哪个设备、位于哪条产线、同类设备在这个数值的历史故障率，以及运维手册里“≥38℃”意味着需要降载运行。它把裸数据翻译成了可行动的情报——但前提是，架构中有一种机制能把LLM的推理结果与实际的设备控制指令对接起来。如果必须由应用层在每次调用LLM前手工拼装上下文、在得到结果后手写几百行代码去下发指令，那“智能”就变成了各应用项目的重复劳动，架构的通用性丢掉一大半。

来源：本书示例场景，相关数值用于说明工程判断，非通用统计结论。
第二个方向来自边缘：边缘智能的实用化。不少工业场景对时延的要求在毫秒或亚秒级——一台高速冲压机若在下一个冲程周期内未能识别振动异常，后果可能是模具损坏；一辆AGV若在500毫秒内未完成避障，可能撞上障碍物。云端往返加上模型推理很容易突破百毫秒，边缘推理虽然也有数十毫秒的消耗，但至少避开了广域网时延的不确定性。这要求架构中有一个位置能在近端运行轻量化模型或规则引擎，并直接或近端影响设备行为。业界常见的分工是“云侧训练、边缘推理、端侧响应”三级协同：云端用全量历史数据训练模型，下发到边缘节点做低延迟推理，端侧只做最后一脚的快速反应。
...
IoT DC3 的开源实践印证了这套分工。DC3 在应用层中实现了一个 Agentic 中心（Agentic Center），基于 Spring AI 框架，内置一组通过工具调用（Tool-Calling）来查设备、读写位号、执行命令的 `@Tool` 接口（资料：[S5]）。Agentic 中心的核心正是智能层的三项职责：它从数据中心读取归一化的位号值进行推理，根据用户意图或异常状态规划动作，通过注入鉴权上下文的指令通道经过网关下发到驱动。该中心兼容 OpenAI API 标准，可接入 GPT、Claude、DeepSeek、通义千问等主流模型（资料：[S5]）。至于 Agentic 中心的工程实现细节——包括工具注册、上下文组装、高耗时操作的异步安全边界——留待第7章展开。

#### 是否每个项目都需要在应用层内划分智能层？

把智能层概念放进架构图，不等于每个 IoT 项目都需要一个与 LLM 交互的页面。它的本质是在应用层中划出一个专有的逻辑区域，负责“理解→决策→执行”的循环。如果这个循环在当前项目中全靠人工完成——运维人员盯着大屏发现问题、打电话让现场操作——那么经典四层架构够用。但一旦项目规模到了上百台设备，或者响应时间要求在秒级以内，人工循环就会成为瓶颈。

智能层的实现方式可以是轻量级的：一个运行在边缘网关上的增强版规则引擎，算法里加了动态阈值学习；也可以是重量级的：一个对接了 LLM、支持多轮对话与多目标规划的 Agent 服务。位置定下来——在应用层内部——实现方式可以随场景弹性选择。这正是架构设计的基本思路：先划层，再定实现，不把实现当架构。
...
判断一个物联网项目是否需要引入这套闭环机制，核心看一条标准：业务中是否出现了“人来不及决策”或“人跨系统拼接上下文成本过高”的瓶颈。如果回答是肯定的，哪怕只在一类设备上，也值得在应用层设计中为智能层留一个位置——哪怕初期只是一个边缘计算节点上的轻量决策规则，它也已经从经典四层迈入了应用层 AI 增强的范畴。智能层的深度实现将在第7章展开，本章先完成概念落位。
...
**智能层（Intelligent Layer）** 是本模型的核心新增层，统一处理三件事：**理解（Understand）**——将位号值序列还原为设备状态与场景语义；**规划（Plan）**——基于规则或模型输出一组动作序列；**执行（Execute）**——通过平台层的命令下发接口把动作送出去，并收回执行反馈。智能层的引入把经典四层中应用层必须承担的决策负担剥离出来，形成一个可复用、与业务场景解耦的决策中枢。它不限定AI技术——可以是大语言模型驱动，也可以是传统规则引擎加实时分析模型，关键在于把“推理”与“执行”的接口标准化。IoT DC3 的智能中心（Agentic Center）是这一层的具体实践：基于 Spring AI 框架连接大语言模型，内置设备查询、位号读写、命令执行等工具（资料：[S6]、[S7]）。
...
![五层架构模型与传统四层架构的对比示意](figures/chapter-02/fig-2-3.png){width=15cm}
...
![图2-4 传统数据模式与智能闭环模式的数据流程对比](figures/chapter-02/fig-2-4.png){width=15cm}
...
![图2-5 智能楼宇节能闭环的组件间交互时序示意图（假设场景）](figures/chapter-02/fig-2-5.png){width=15cm}

*图2-5 智能楼宇节能闭环的组件间交互时序示意图（假设场景）*
...
![图2-6 智能层在闭环中的推理-规划-执行分工图](figures/chapter-02/fig-2-6.png){width=15cm}

*图2-6 智能层在闭环中的推理-规划-执行分工图，展示认知环节与平台层基础设施的边界。*
...
![图2-5 IoT DC3 五大中心逻辑关系](figures/chapter-02/fig-2-5-2.png){width=15cm}
...
![Data中心数据流示意：从设备采集到时序存储](figures/chapter-02/fig-2-8.png){width=15cm}
...
![Agentic 中心决策执行流程](figures/chapter-02/fig-2-12.png){width=15cm}
...
![五大中心协同流程时序图（智能灌溉场景）](figures/chapter-02/fig-2-7.png){width=15cm}
...
| 维度 | 关系数据库（MySQL/PostgreSQL） | 时序数据库（InfluxDB/TimescaleDB） |
|------|-------------------------------|------------------------------------|
...
![图8-2 物联网纵深防御多层模型](figures/chapter-08/fig-8-2.png){width=15cm}
...
![图8-2 TLS 1.3与EDHOC握手流程对比](figures/chapter-08/fig-08-02.png){width=15cm}

*图8-2 展示TLS 1.3双向认证与EDHOC轻量认证的握手流程对比。TLS需要两次往返且传输证书，EDHOC仅需3条消息，体积更小。*
...
![图8-1 安全启动多层信任链流程](figures/chapter-08/fig-08-01-2.png){width=15cm}
...
![图8-11 防篡改设计的纵深防线](figures/chapter-08/fig-8-11.png){width=15cm}
...
![图8-1 TLS 1.3握手流程](figures/chapter-08/fig-08-01-3.png){width=15cm}
...
![图8-8 CoAP 的 OSCORE 安全处理流程示意](figures/chapter-08/fig-08-08.png){width=15cm}

*OSCORE 处理模块先将 CoAP 请求载荷加密，再用 Client 与 Server 预共享的密钥计算基于 HMAC 的 MAC，然后将加密结果、密钥标识（Kid）和序列号打包为一条新的 CoAP 请求。Server 端解密并校验 MAC，同时检查序列号是否已在滑动窗口内。设备睡眠后重新上线，序列号从上次持久化的递增位点继续，无需重新握手。*
...
![物联网微隔离架构示意图](figures/chapter-08/figure-8-7.png){width=15cm}
...
![图8-5 密钥生命周期管理与云边协同流程](figures/chapter-08/fig-key-lifecycle.png){width=15cm}
...
![数据脱敏与匿名化决策流程](figures/chapter-08/fig-8-3.png){width=15cm}
...
![多租户隔离强度对比](figures/chapter-08/table-8-5.png){width=15cm}
...
![智能家居多租户系统架构](figures/chapter-08/fig-8-10.png){width=15cm}
...
- **资源隔离压力测试**：对租户A的容器发送数千个并发请求，同时监控租户B的接口响应时延。如果租户B的时延因为租户A的负载而飙升，说明计算资源隔离没有真正生效。测试通过判据是“响应时延偏差不超过基准的20%”。
...
![本章参考资料分类导图](figures/chapter-08/fig-8-7.png){width=15cm}
...
![数字孪生五维模型架构图](figures/chapter-10/fig-10-2.png){width=15cm}

*五维模型架构图。连接层（CN）是数字孪生能否实时运转的关键：它决定了物理实体与虚拟模型之间的同步精度和指令可达性。数据与服务层和知识层则提供了从数据到决策的持续演进能力。*
...
大多数工厂目前停留在L1到L2之间，少数领先企业已经迈入L3。从L2到L3的跨越，通常需要时序数据库和规则引擎的支撑——这正是IoT DC3工业平台在10.3节会重点展开的内容。
...
![图10-3 三类维护策略长期运营效率对比（示意）](figures/chapter-10/fig-10-3.png){width=15cm}
...
关键参数说明（资料：[S3]）：
- `interval: "PT5S"`：驱动每 5 秒向该设备发起一次轮询采集。
...
![图10-15 原始数据与清洗后数据对比](figures/chapter-10/fig-10-15.png){width=15cm}
...
![图10-10 AI模型部署架构（从训练到边缘推理）](figures/chapter-10/fig-10-10.png){width=15cm}
...
![图10-4 预测性维护闭环数据流（示意）](figures/chapter-10/fig-10-04-2.png){width=15cm}

*图10-4 预测性维护闭环的典型数据流（示意）：从原始振动信号到维护工单生成，经过五层转换，每层都有明确的输入输出格式和系统边界。图中阈值和窗口为示例，需根据具体设备校准。*
...
![图10-7 IoT DC3平台微服务架构](figures/chapter-10/fig-10-06.png){width=15cm}
...
![图10-4 IoT DC3设备接入与数据采集数据流](figures/chapter-10/fig-10-04-3.png){width=15cm}

*图10-4 示意SMT产线场景下IoT DC3从设备接入到监控大屏的数据流。*
...
![图10-4 规则引擎告警触发与预测性维护工单闭环流程](figures/chapter-10/fig-10-04-4.png){width=15cm}

*图10-4 展示了从温度数据判定到AI推理、工单创建、通知发送，以及基于工单状态回环的完整闭环流程。橙色部分表示升级路径，是在等待回复超时后触发的二次决策。*
...
![图10-14 本章知识体系图谱](figures/chapter-10/fig-10-14.png){width=15cm}
...
![图11-1 V2X通信技术选型决策路径图](figures/chapter-11/fig-11-1.png){width=15cm}

*图11-1 V2X通信技术选型决策路径图*
...
![图11-2 RSU与OBU通信流程](figures/chapter-11/fig-11-2-rsu-obu-sequence.png){width=15cm}

*图11-2 RSU与OBU通信流程图。展示了信号机、路侧雷达、RSU、OBU、车载HMI和ADAS域控制器之间的消息流向和时序。*
...
![图11-3 智能路灯杆功能图（假设场景）](figures/chapter-11/fig-11-3.png){width=15cm}

*图11-3 智能路灯杆功能图（假设场景），展示五类模块与边缘计算盒的数据流向关系。*
...
![图11-4 城市应急响应物联网架构](figures/chapter-11/fig-11-4.png){width=15cm}

*图11-4 展示城市应急响应系统从感知到指挥的四层职责边界与主数据流向。*
...
![图11-5 火灾应急响应事件流转时序（假设场景）](figures/chapter-11/fig-11-5.png){width=15cm}

*图11-5 自动化链路的关键在于边缘节点完成本地判定（第2步），云端协同层完成自动事件路由（第4步），两地均无人工转接环节。*
...
![城市物联网百万级接入系统架构图](figures/chapter-11/figure-11-5.png){width=15cm}
...
![图11-5 城市物联网消息队列与数据流处理架构](figures/chapter-11/fig-11-5-2.png){width=15cm}

*图11-5 展示设备层、消息队列、流处理层与存储服务层之间的数据流动。设备上报到Kafka，Flink消费后聚合写入Redis/时序DB；AI推理微服务通过RocketMQ接收控制指令。*
...
![图11-6 云边协同分层架构示意](figures/chapter-11/fig-11-6.png){width=15cm}

*图11-6 云边协同分层架构示意：设备层数据经路侧RSU实时处理后，聚合统计上传至区域汇聚节点，汇聚节点运行短期预测模型，并与云端进行训练数据交换。*
...
![图11-7 LSTM交通流预测模型架构图](figures/chapter-11/fig-11-7.png){width=15cm}
...
![图11-8 十字路口强化学习框架](figures/chapter-11/fig-11-08.png){width=15cm}
...
![图11-9 能耗数据对比图（智能照明 vs 传统照明，假设场景）](figures/chapter-11/fig-11-9.png){width=15cm}
...
![图11-5 智慧交通系统集成部署工程检查流程](figures/chapter-11/fig-11-5-3.png){width=15cm}

*图11-5 智慧交通系统集成部署工程检查流程*
...
![新区城市大脑系统部署架构图](figures/chapter-11/fig11-10.png){width=15cm}
...
![图12-2 作物病害识别CNN处理流程（架构示意）](figures/chapter-12/fig-12-2.png){width=15cm}
...
![图12-4 LoRa / NB-IoT / Sigfox 技术维度对比雷达图](figures/chapter-12/fig-12-04.png){width=15cm}
...
![图12-4 智慧果园综合监测系统架构（示意）](figures/chapter-12/fig-12-04-2.png){width=15cm}
...
![图13-1 中心化物联网架构的信任困境](figures/chapter-13/fig-13-1.png){width=15cm}
...
![图13-2 区块链与物联网融合的信任三角](figures/chapter-13/fig-13-2.png){width=15cm}
...
![图13-2 设备DID注册与验证流程](figures/chapter-13/fig-13-2-2.png){width=15cm}
...
![图13-2 数据上链流程图](figures/chapter-13/fig-13-2-3.png){width=15cm}
...
![数据验证与溯源机制时序图](figures/chapter-13/figure-13-4.png){width=15cm}
...
![图13-6 供应链核心痛点与区块链价值点映射](figures/chapter-13/fig-13-6.png){width=15cm}
...
![图13-8 基于区块链的供应链溯源系统分层架构](figures/chapter-13/fig-13-8.png){width=15cm}
...
![图13-3 中心化 vs 去中心化物联网架构对比图](figures/chapter-13/fig-13-03.png){width=15cm}
...
![图13-4 AI+区块链+物联网三角范式关系图](figures/chapter-13/fig-13-4.png){width=15cm}
...
**后量子密码学的逼近**。物联网设备广泛采用的椭圆曲线数字签名（ECDSA）在量子计算面前脆弱的假设已经被学术界反复验证。NIST正在推进的后量子密码（PQC）标准化进程中，基于格的方案（如CRYSTALS-Kyber和CRYSTALS-Dilithium）是主要候选方向。对于微控制器级别的物联网节点，这些算法的密钥尺寸和签名计算开销是硬约束，行业正在探索轻量级后量子签名方案（如基于哈希的XMSS）在TEE或安全飞地中的集成路径。工程判断：在可预见的阶段，区块链物联网项目应从硬件层面预置PQC可升级的密钥存储模块，避免大规模部署后陷入迁移困境。

**6G网络与区块链的原生集成**。ITU-T关于IMT-2030的早期讨论中，已出现将分布式信任机制嵌入网络协议栈的设想。6G的设计目标是支持极低时延的机器间协作，这要求信任不再是上层叠加，而是网络原生能力。区块链（或其变体DAG）可能以“网络原生信任层”的形式存在——通过网络切片配给专用的共识节点资源，或利用感知通信一体化实现设备位置与链上身份的锚定。这一趋势目前处于学术预研阶段，但长期架构规划中需要预留轻量级跨域身份接口。
